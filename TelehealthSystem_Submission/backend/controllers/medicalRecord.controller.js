...
// Bệnh nhân lấy danh sách hồ sơ bệnh án của mình
const getMyMedicalRecords = async (req, res) => {
  try {
    const userId = req.user._id;

    // Tìm Patient document tương ứng với user hiện tại
    const patientDoc = await Patient.findOne({ user: userId });
    if (!patientDoc) {
    ...
    // Lấy danh sách hồ sơ, chỉ sort và paginate
    const records = await MedicalRecord.find(filter)
      .populate('appointment', 'date time status')
      .sort({ createdAt: -1 })
      .skip(skip)
    ...
    // Map snapshot bác sĩ, chuyên khoa, cơ sở
    const recordsWithNames = await Promise.all(records.map(async r => {
      let specialtyName = '-', locationName = '-';
    ...
    console.error("getMyMedicalRecords error:", err);
