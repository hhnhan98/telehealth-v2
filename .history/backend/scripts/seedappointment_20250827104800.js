...
const main = async () => {
  ...
  try {
    ...
    // --- Xóa tất cả appointment hôm nay ---
    ...
    // --- Tạo 1 appointment cho mỗi trạng thái ---
    for (let i = 0; i < statuses.length; i++) {
      let hour = 9 + i; // bắt đầu từ 9h, mỗi trạng thái 1 giờ cách nhau
      let datetimeVN = nowVN.hour(hour).minute(0).second(0).millisecond(0);

      // Nếu slot đã qua, tăng giờ cho đến slot còn trong ngày
      while (datetimeVN.isBefore(nowVN) && hour <= 16) {
        hour++;
        datetimeVN = nowVN.hour(hour).minute(0).second(0).millisecond(0);
      }

      if (datetimeVN.isBefore(nowVN)) {
        console.log(`⚠ Không còn slot trống cho trạng thái ${statuses[i]}`);
        continue;
      }
