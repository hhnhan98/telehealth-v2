...
// Giờ làm việc (theo giờ VN, UTC+7)
const WORK_HOURS = [
  ...
  try {
    ...
    for (let doctor of doctors) {
      for (let i = 0; i < 7; i++) { // 7 ngày từ hôm nay
        ...
        dateObj.setUTCHours(0,0,0,0); // reset về 00:00 UTC
        dateObj.setDate(today.getDate() + i);

        const dateStr = formatDate(dateObj);

        // Schedule slots (giữ giờ VN, convert sang UTC khi tạo Appointment)
        const slots = WORK_HOURS.map(time => ({ time, isBooked: false }));
        await Schedule.create({ doctorId: doctor._id, date: dateStr, slots });

        // Hôm nay: tạo 2 appointment để test dashboard
        if (i === 0) {
          for (let j = 0; j < 2; j++) {
            const patient = patientUsers[j % patientUsers.length];
            const [hour, minute] = WORK_HOURS[j].split(":");

            // Chuyển giờ VN -> UTC (trừ 7h)
            const datetimeUTC = new Date(dateObj);
            datetimeUTC.setUTCHours(parseInt(hour) - 7, parseInt(minute), 0, 0);
        ...
        // Các ngày khác trong tuần: tạo 1 appointment
        if (i > 0) {
          const patient = patientUsers[i % patientUsers.length];
          const [hour, minute] = WORK_HOURS[0].split(":");
