...
const DOCTOR_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9'; // token bác sĩ
const PATIENT_TOKEN = 'YOUR_PATIENT_TOKEN_HERE'; // token bệnh nhân test
...
// ----- Tạo appointment hợp lệ cho hôm nay với timezone VN -----
const createAppointment = async (doctor, vnTime = '10:00', reason = 'Test dashboard appointment') => {
  try {
    ...
    // Chuyển giờ VN sang UTC để lưu vào DB
    const vnDatetime = dayjs().tz('Asia/Ho_Chi_Minh').hour(hour).minute(minute).second(0);
    ...
    const res = await axios.post(
      `${API_BASE}/appointments`,
      {
        ...
        datetime: utcDatetime,           // ✅ gửi datetime chuẩn UTC
    ...
    if (res.data.success) {
      ...
      console.log('>>> Appointment datetime stored in DB (UTC):', res.data.data.appointment.datetime);
