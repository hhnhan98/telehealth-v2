...
async function seedDemoData() {
  try {
    ...
    // Track slot đã dùng: { doctorId: Set('YYYY-MM-DD HH:mm') }
    const usedSlots = {};
    doctors.forEach(doc => usedSlots[doc.doctor._id] = new Set());
    ...
    for (let doc of doctors) {
      for (let d = new Date(startSeedDate); d <= endSeedDate; d.setDate(d.getDate() + 1)) {
        for (let hour = 9; hour <= 11; hour++) {
          ...
          if (usedSlots[doc.doctor._id].has(slotKey)) continue;
          usedSlots[doc.doctor._id].add(slotKey);
    ...
    for (let pat of patients) {
      ...
      while (added < 3) {
        ...
        const dayOffset = Math.floor(Math.random() * ((endSeedDate.getTime() - startSeedDate.getTime())/(1000*60*60*24) + 1));
        randDay.setDate(randDay.getDate() + dayOffset);
