// backend/seed/seedDemoDataUTCFinal.js
...
async function seedDemoData() {
  try {
    ...
    for (let doctor of doctors) {
      // --- 6a. Tạo ít nhất 1 appointment hôm nay ---
      const patientToday = patientUsers[Math.floor(Math.random() * patientUsers.length)];
      const hourMinToday = WORK_HOURS[Math.floor(Math.random() * WORK_HOURS.length)];
      const [hourToday, minuteToday] = hourMinToday.split(":");
      const datetimeTodayUTC = new Date(Date.UTC(
        today.getFullYear(),
        today.getMonth(),
        today.getDate(),
        parseInt(hourToday) - 7,
        parseInt(minuteToday),
        0
      ));
      await Appointment.create({
        ...
        datetime: datetimeTodayUTC,
        status: 'confirmed',
        isVerified: true
      });
    }

    // --- 6b. Tạo thêm appointment ngẫu nhiên cho bệnh nhân ---
    ...
    console.log("^.^ Seed demo data UTC Final thành công!");
    process.exit();
