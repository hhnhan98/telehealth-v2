...
async function seed() {
  try {
    ...
    for (const doc of doctors) {
      ...
      console.log(`- ${user.fullName} | Specialty: ${specialty.name} | Location: ${location.name} | Doctor ID: ${doc._id}`);
    }
    ...
    console.log('🎉 Seed completed! Bạn có thể kiểm tra API getDoctorsBySpecialtyAndLocation với các ID đã log.');
    process.exit(0);
