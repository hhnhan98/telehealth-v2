...
async function seedDemoData() {
  try {
    ...
    // ===== 5. Tạo Schedule cho từng bác sĩ (trong vòng 7 ngày) =====
    for (let doctor of doctors) {
      for (let offset = -3; offset <= 3; offset++) { // 3 ngày trước/sau
    ...
    const usedSlots = {}; // tránh trùng slot
    ...
    for (let patient of patientUsers) {
      ...
      while (count < 5) {
        ...
        if (usedSlots[key]) continue; // tránh trùng
        usedSlots[key] = true;
        ...
        const newAppt = await Appointment.create({
          ...
          otp: status === "pending" ? otp : null,
          otpExpiresAt: status === "pending" ? otpExpiresAt : null
        });

        // Cập nhật slot trong Schedule là booked
        const schedule = await Schedule.findOne({ doctorId: doctor._id, date: dateStr });
        const slot = schedule.slots.find(s => s.time === hourMin);
