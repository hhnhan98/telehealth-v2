...
const main = async () => {
  ...
  try {
    ...
    // --- Xóa tất cả appointment hôm nay của doctor + patient (demo + thực) ---
    await Appointment.deleteMany({ doctor: doctorId, patient: patientId, date: todayVNStr });
    console.log('✅ Đã xóa toàn bộ appointment cũ của doctor + patient');
    ...
    for (let status of statuses) {
      // --- Chọn ngẫu nhiên giờ và phút 30 phút/lượt từ 08:00 → 16:30 ---
      let slotFound = false;
      for (let attempt = 0; attempt < 50; attempt++) { // tối đa 50 lần thử
        ...
        slotFound = true;
        break; // tìm được slot hợp lệ, thoát vòng lặp
      }
