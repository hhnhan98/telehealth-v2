...
async function seed() {
  try {
    ...
    const specialties = await Promise.all(specialtiesData.map(d => new Specialty(d).save()));
    ...
    const doctors = await Promise.all(doctorData.map(d => new Doctor(d).save()));
