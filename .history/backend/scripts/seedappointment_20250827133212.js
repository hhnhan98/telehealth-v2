...
const main = async () => {
  ...
  try {
    ...
    console.log(`🔹 Seed appointment demo cho doctorUser ${doctorUserId} và patientUser ${patientUserId}`);

    const nowVN = dayjs().tz('Asia/Ho_Chi_Minh');

    // --- Xóa toàn bộ appointment cũ của doctor (quá khứ + hiện tại + tương lai) ---
    await Appointment.deleteMany({ doctor: doctorId });
    console.log('✅ Đã xóa toàn bộ appointment cũ của bác sĩ');
    ...
    for (let status of statuses) {
      ...
      for (let attempt = 0; attempt < 50; attempt++) {
        ...
        const exists = await Appointment.findOne({
          doctor: doctorId,
          date: datetimeVN.format('YYYY-MM-DD'),
          time: timeStr,
        });
        if (exists) continue;
