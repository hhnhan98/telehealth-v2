...
async function seedDemoData() {
  try {
    ...
    // --- Users (plaintext passwords, pre-save hook sẽ hash) ---
    ...
    // --- Specialties ---
    const spec1 = await Specialty.create({ name: 'Chuyên khoa 1', locations: [loc1._id, loc2._id] });
    const spec2 = await Specialty.create({ name: 'Chuyên khoa 2', locations: [loc1._id] });
    const spec3 = await Specialty.create({ name: 'Chuyên khoa 3', locations: [loc2._id] });
    const spec4 = await Specialty.create({ name: 'Chuyên khoa 4', locations: [loc2._id] });
    console.log('✅ Specialties seeded');

    // --- Gán specialty vào Location.specialties ---
    await Location.findByIdAndUpdate(loc1._id, { specialties: [spec1._id, spec2._id] });
    await Location.findByIdAndUpdate(loc2._id, { specialties: [spec1._id, spec3._id, spec4._id] });
    ...
    // --- Gán doctors vào Specialty.doctors ---
    await Specialty.findByIdAndUpdate(spec1._id, { doctors: [doctors[0]._id] });
    await Specialty.findByIdAndUpdate(spec2._id, { doctors: [doctors[1]._id] });
    await Specialty.findByIdAndUpdate(spec3._id, { doctors: [doctors[2]._id] });
    await Specialty.findByIdAndUpdate(spec4._id, { doctors: [doctors[3]._id] });
