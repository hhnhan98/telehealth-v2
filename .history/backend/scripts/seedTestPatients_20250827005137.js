...
async function seed() {
  try {
    ...
    // 1. Tạo specialty & location test nếu chưa có
    let specialty = await Specialty.findOne({ name: 'Nội tổng quát' });
    if (!specialty) specialty = await Specialty.create({ name: 'Nội tổng quát' });

    let location = await Location.findOne({ name: 'Phòng khám Test' });
    if (!location) location = await Location.create({ name: 'Phòng khám Test' });
    ...
    if (!appointment) {
      ...
      appointment.location = location._id;
      appointment.datetime = new Date();
      await appointment.save();
    }

    // 5. Tạo medical record liên kết
