...
async function seedDemoData() {
  try {
    ...
    const locations = await Location.insertMany([
      ...
      { name: "Cơ sở Hồ Chí Minh", address: "456 Nguyễn Huệ, TP.HCM" },
      { name: "Cơ sở Đà Nẵng", address: "789 Bạch Đằng, Đà Nẵng" },
    ]);
    ...
    // ===== 4. Gán Doctor (tham chiếu User, Location, Specialty) =====
    const doctors = await Doctor.insertMany([
      { user: doctorUsers[0]._id, name: doctorUsers[0].fullName, specialties: [specialties[0]._id, specialties[1]._id], location: locations[0]._id },
      { user: doctorUsers[1]._id, name: doctorUsers[1].fullName, specialties: [specialties[1]._id, specialties[2]._id], location: locations[1]._id },
      { user: doctorUsers[2]._id, name: doctorUsers[2].fullName, specialties: [specialties[0]._id, specialties[2]._id], location: locations[2]._id },
    ]);
    ...
    for (let p of patientUsers) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // random ngày trong khoảng [-7, +7] ngày
        const dayOffset = Math.floor(Math.random() * 15) - 7;
        let date = new Date();
        date.setDate(date.getDate() + dayOffset);

        // random giờ trong khoảng hành chính
        ...
        if (usedDates.has(key)) continue; // tránh trùng
        usedDates.add(key);
