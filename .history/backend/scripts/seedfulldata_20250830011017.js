...
const seedData = async () => {
  try {
    ...
    for (let i = 0; i < 10; i++) {
    // Chọn random specialty và lấy location của specialty đó
    ...
    // Kiểm tra nếu đã tồn tại user này
    ...
    console.log(`👨‍⚕️ ${doctorUsers.length} doctor users created. Post-save hook sẽ tự tạo Doctor document.`);


    // ----------- Patients (20 Users, Patient tự tạo nhờ post-save hook) -----------
    const patientUsers = [];
    ...
    if (!existingUser) {
        ...
        patientUsers.push(existingUser);
    }
    }

    console.log(`👨‍👩‍👧 ${patientUsers.length} patient users created. Patient document được tạo tự động nhờ hook.`);
