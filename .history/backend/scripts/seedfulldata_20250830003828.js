...
const seedData = async () => {
  try {
    ...
    const specialtyNames = ['Nội', 'Ngoại', 'Răng Hàm Mặt', 'Da Liễu', 'Mắt'];
    const specialties = [];
    for (const loc of locations) {
      for (const name of specialtyNames) {
        const spec = await Specialty.create({ name, location: loc._id });
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const loc = locations.find(l => l._id.equals(spec.location));
      const email = `doctor${i}@demo.com`;
      ...
      const doctor = await Doctor.create({
        ...
        specialty: spec._id,
        location: loc._id,
        bio: `Bác sĩ ${i} chuyên khoa ${spec.name} tại ${loc.name}`
      });

      doctorDocs.push(doctor);
    }
    console.log(`👨‍⚕️ ${doctorDocs.length} Doctors created`);
