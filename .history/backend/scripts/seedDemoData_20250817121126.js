...
async function seedDemoData() {
  try {
    ...
    // ===== 4. Tạo Doctor (tham chiếu User + Location + Specialty) =====
    ...
    for (let p of patientUsers) {
      ...
      for (let i = 0; i < 5; i++) {
        ...
        // random giờ trong khung giờ hành chính
        ...
        if (usedSlots.has(key)) continue; // tránh trùng
        usedSlots.add(key);
        ...
        appointments.push({
          ...
          datetime: date,
          patient: p._id,
          reason: `Khám ${specialty.name}`,
          status: statuses[i % statuses.length],
          isVerified: statuses[i % statuses.length] !== "pending",
        });
      }
    }
