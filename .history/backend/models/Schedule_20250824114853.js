/**
 * @swagger
 * components:
 *   schemas:
 *     Slot:
 *       type: object
 *       description: Thông tin một khung giờ khám trong ngày
 *       properties:
 *         time:
 *           type: string
 *           pattern: "^\\d{2}:\\d{2}$"
 *           description: Giờ khám định dạng HH:mm
 *           example: "08:30"
 *         isBooked:
 *           type: boolean
 *           description: Trạng thái khung giờ đã được đặt hay chưa
 *           default: false
 *       required:
 *         - time
 *
 *     Schedule:
 *       type: object
 *       description: Lịch làm việc của một bác sĩ trong ngày
 *       properties:
 *         _id:
 *           type: string
 *           description: ID tự sinh bởi MongoDB
 *         doctorId:
 *           type: string
 *           description: ID tham chiếu Doctor
 *           example: "64c123abc456def7890ccc56"
 *         date:
 *           type: string
 *           description: Ngày làm việc (định dạng YYYY-MM-DD)
 *           example: "2025-08-25"
 *         slots:
 *           type: array
 *           items:
 *             $ref: '#/components/schemas/Slot'
 *         createdAt:
 *           type: string
 *           format: date-time
 *         updatedAt:
 *           type: string
 *           format: date-time
 *       required:
 *         - doctorId
 *         - date
 */

const mongoose = require('mongoose');

const SlotSchema = new mongoose.Schema({
  time: {
    type: String,
    required: [true, 'Thời gian slot là bắt buộc'], // ví dụ "08:00"
    match: [/^\d{2}:\d{2}$/, 'Định dạng thời gian không hợp lệ'],
  },
  isBooked: {
    type: Boolean,
    default: false,
  },
}, { _id: false }); // không cần _id riêng cho slot

const ScheduleSchema = new mongoose.Schema({
  doctorId: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'Doctor',
    required: [true, 'doctorId là bắt buộc'],
  },
  date: {
    type: String,
    required: [true, 'Ngày là bắt buộc'],
    match: [/^\d{4}-\d{2}-\d{2}$/, 'Định dạng ngày phải là YYYY-MM-DD'],
  },
  slots: {
    type: [SlotSchema],
    default: [],
  },
}, { timestamps: true });

// Đảm bảo mỗi doctor chỉ có 1 schedule cho 1 ngày
ScheduleSchema.index({ doctorId: 1, date: 1 }, { unique: true });

module.exports = mongoose.model('Schedule', ScheduleSchema);