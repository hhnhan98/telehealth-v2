...
userSchema.pre('save', async function (next) {
  try {
    // Luôn trim password & hash nếu mới tạo hoặc password thay đổi
    if (this.isNew || this.isModified('password')) {
      const cleanPassword = String(this.password).trim(); // tránh khoảng trắng
      const hash = await bcrypt.hash(cleanPassword, SALT_ROUNDS);
      this.password = hash;
    }
    // Đồng thời trim email để tránh login fail do khoảng trắng
