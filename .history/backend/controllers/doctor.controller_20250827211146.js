...
const getDoctorAppointments = async (req, res) => {
  try {
    // 1️⃣ Tìm doctor dựa vào user đăng nhập
    const doctor = await Doctor.findOne({ user: req.user._id });
    if (!doctor) return responseError(res, 'Không tìm thấy doctor', 404);

    // 2️⃣ Lấy danh sách appointments của doctor, trạng thái confirmed/completed
    const appointments = await Appointment.find({
      doctor: doctor._id,
      status: { $in: ['confirmed', 'completed'] }
    })
      ...
      .populate('specialty', 'name')
      .sort({ datetime: 1 })
      .lean({ virtuals: true }); // chuyển luôn sang object có virtuals
