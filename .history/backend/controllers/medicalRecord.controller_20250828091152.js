...
const createMedicalRecord = async (req, res) => {
  try {
    ...
    // Mỗi appointment chỉ 1 medical record
    ...
    // patient nếu không gửi thì lấy từ appointment
    const patientId = patient || appt.patient;
    if (!isValidObjectId(patientId)) return responseError(res, 'patient id không hợp lệ', 400);
...
const getMedicalRecordsByPatient = async (req, res) => {
  try {
    ...
    if (!patientId || !isValidObjectId(patientId)) return responseError(res, 'ID bệnh nhân không hợp lệ', 400);

    const role = (req.user.role || '').toLowerCase();

    // Nếu user là patient -> chỉ được lấy chính mình
...
const updateMedicalRecord = async (req, res) => {
  try {
    ...
    // Only allow a whitelist of fields to update
    const allowed = [
      'height', 'weight', 'bp', 'pulse', 'bmi',
      'symptoms', 'diagnosis', 'notes', 'prescriptions',
      'conclusion', 'careAdvice'
    ];
