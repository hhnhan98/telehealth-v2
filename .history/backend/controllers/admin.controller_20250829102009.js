...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    if (role === 'doctor' && (!specialty || !location)) throw new Error('Bác sĩ phải chọn chuyên khoa và cơ sở y tế');
    ...
    let doctor = null;
    if (role === 'doctor') {
      doctor = await Doctor.create([{ user: newUser._id, specialty, location }], { session });
    }

    await session.commitTransaction();
    session.endSession();

    const populatedDoctor = doctor
      ? await Doctor.findById(doctor[0]._id)
          .populate('specialty', 'name')
          .populate('location', 'name')
      : null;
...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'doctor' && (update.specialty || update.location)) {
      ...
      if (doctor) {
        if (update.specialty) doctor.specialty = update.specialty;
        if (update.location) doctor.location = update.location;
...
exports.createDoctor = async (req, res) => {
  ...
  try {
    ...
    // Validate Specialty và Location tồn tại
    const foundSpecialty = await Specialty.findById(specialty).session(session);
    if (!foundSpecialty) throw new Error('Specialty không tồn tại');

    const foundLocation = await Location.findById(location).session(session);
    if (!foundLocation) throw new Error('Location không tồn tại');

    // Validate specialty thuộc location
    if (foundSpecialty.location.toString() !== location)
      throw new Error('Specialty không thuộc location đã chọn');
    ...
    const populatedDoctor = await Doctor.findById(doctor[0]._id)
      .populate('user', 'fullName email avatar phone birthYear role')
      .populate('specialty', 'name')
      .populate('location', 'name');
...
exports.updateDoctor = async (req, res) => {
  ...
  try {
    ...
    // Validate specialty nếu có
    if (specialty) {
      const spec = await Specialty.findById(specialty).session(session);
      if (!spec) throw new Error('Specialty không tồn tại');

      // Nếu có location đổi kèm, kiểm tra specialty thuộc location mới
      if (location && spec.location.toString() !== location)
        throw new Error('Specialty không thuộc location đã chọn');
      // Nếu location không đổi, kiểm tra specialty vẫn thuộc location hiện tại
      if (!location && spec.location.toString() !== doctor.location.toString())
        throw new Error('Specialty không thuộc location hiện tại');
    ...
    // await doctor.save({ session });
    if (location) {
      // Nếu doctor hiện tại có specialty, kiểm tra hợp lệ với location mới
      if (doctor.specialty) {
        const spec = await Specialty.findById(doctor.specialty).session(session);
        if (spec && spec.location.toString() !== location) {
          doctor.specialty = null; // reset specialty vì không hợp lệ
          // hoặc throw new Error('Specialty hiện tại không thuộc location mới');
        }
      }
    ...
    // Populate doctor trước khi trả về
