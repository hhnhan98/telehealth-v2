...
//       } catch (err) {
//         console.error('*** Tạo Patient document thất bại ', err.message);
//       }
//     }
...
exports.register = async (req, res) => {
  try {
    ...
    // --- Tạo Patient document nếu role = 'patient'
    if (role === 'patient') {
      await Patient.create({ user: newUser._id });
    }

    // --- Tạo Doctor document nếu role = 'doctor'
    if (role === 'doctor') {
      // Tạo Doctor document
      ...
      if (specialtyDoc) {
        specialtyDoc.doctors = specialtyDoc.doctors || [];
        if (!specialtyDoc.doctors.includes(newUser._id)) {
          specialtyDoc.doctors.push(newUser._id);
          await specialtyDoc.save();
        }
      ...
      if (locationDoc) {
        locationDoc.doctors = locationDoc.doctors || [];
        if (!locationDoc.doctors.includes(newUser._id)) {
          locationDoc.doctors.push(newUser._id);
          await locationDoc.save();
        }
