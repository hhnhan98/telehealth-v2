...
const Location = require('../models/Specialty');
...
exports.register = async (req, res) => {
  try {
    ...
    if (role === 'doctor') {
      ...
      // Kiểm tra Specialty có thuộc Location không
      // const spec = await Specialty.findOne({ _id: specialty, location });
      const spec = await Specialty.findOne({ _id: specialty, locations: { $in: [location] } });
      if (!spec) return responseError(res, 'Chuyên khoa không hợp lệ cho cơ sở y tế này');
    }
