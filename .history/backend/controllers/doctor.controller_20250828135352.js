...
const getDoctorAppointments = async (req, res) => {
  try {
    ...
    const appointments = await Appointment.find({ doctor: doctor._id })
      .populate({ 
        path: 'patient', 
        populate: { path: 'user', select: 'fullName email phone avatar', strictPopulate: false }, 
        strictPopulate: false 
      })
      .populate({ 
        path: 'doctor', 
        populate: { path: 'user', select: 'fullName email', strictPopulate: false }, 
        strictPopulate: false 
      })
      .populate('specialty', 'name')
      .populate('location', 'name')
    ...
    responseSuccess(res, 'Lấy danh sách appointments thành công', appointments);
  } catch (err) {
...
const getAppointmentDetail = async (req, res) => {
  try {
    ...
    const appointment = await Appointment.findById(id)
      .populate({ 
        path: 'patient', 
        populate: { path: 'user', select: 'fullName email phone avatar birthYear', strictPopulate: false }, 
        strictPopulate: false 
      })
      .populate({ 
        path: 'doctor', 
    ...
    if (!appointment) return responseError(res, 'Không tìm thấy cuộc hẹn', 404);
    
    // Kiểm tra quyền: chỉ doctor hiện tại được xem
    const doctor = await Doctor.findOne({ user: req.user._id });
    if (!doctor) return responseError(res, 'Không tìm thấy doctor', 404);
    ...
    responseSuccess(res, 'Lấy chi tiết appointment thành công', appointment);
  } catch (err) {
...
const createMedicalReceipt = async (req, res) => {
  try {
    ...
    const { id } = req.params;
    const appointment = await Appointment.findById(id);
    if (!appointment) return responseError(res, 'Khôsng tìm thấy lịch hẹn', 404);
    if (appointment.doctor.toString() !== doctor._id.toString())
      return responseError(res, 'Bạn không có quyền tạo phiếu khám cho lịch hẹn này', 403);

    appointment.medicalReceipt = req.body; 
    appointment.status = 'completed';

    // **Đảm bảo save đồng bộ**
    await appointment.save();

    // **Tạo record riêng trong collection MedicalRecord**
