...
const getMedicalRecordsByPatient = async (req, res) => {
  try {
    ...
    // Map lại thông tin doctor, specialty, location từ snapshot
    const recordsWithSnapshot = await Promise.all(
      records.map(async (r) => {
        let doctorName = r.doctorSnapshot?.fullName || '-';
        let specialtyName = '-';
        let locationName = '-';

        // Nếu có snapshot specialty/location là ObjectId, populate tên
        if (r.doctorSnapshot?.specialty) {
          const spec = await Specialty.findById(r.doctorSnapshot.specialty).select('name').lean();
          specialtyName = spec?.name || '-';
        }
        if (r.doctorSnapshot?.location) {
          const loc = await Location.findById(r.doctorSnapshot.location).select('name').lean();
