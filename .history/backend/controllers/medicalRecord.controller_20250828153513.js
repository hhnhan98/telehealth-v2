...
const getMedicalRecordsByPatient = async (req, res) => {
  try {
    ...
    const total = await MedicalRecord.countDocuments(filter);
    const records = await MedicalRecord.find(filter)
      .populate('appointment', 'date time status') // chỉ lấy thời gian và trạng thái
      .populate('patient', 'fullName birthYear address phone') // optional, snapshot cũng có
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();

    // Map snapshot của bác sĩ vào response
    const recordsWithDoctor = records.map(r => ({
      ...r,
      doctor: {
        fullName: r.doctorSnapshot?.fullName || '-',
        specialty: r.doctorSnapshot?.specialty?.name || r.doctorSnapshot?.specialty || '-',
        location: r.doctorSnapshot?.location?.name || r.doctorSnapshot?.location || '-'
      }
    }));
