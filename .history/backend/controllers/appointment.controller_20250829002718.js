...
const getSpecialtiesByLocation = async (req, res) => {
  try {
    ...
    const specialties = await Specialty.find({ location: locationId }).sort({ name: 1 });
...
//     // Tạo OTP và Appointment với datetime chuẩn UTC
//     const otp = generateOTP();
//     const datetimeUTC = toUTC(`${date} ${time}`);
...
//     const appointment = await Appointment.findById(id);
//     if (!appointment) return responseError(res, 'Không tìm thấy lịch hẹn', 404);
//     if (appointment.patient.toString() !== patientId.toString()) return responseError(res, 'Không có quyền hủy', 403);
//     if (!['pending', 'confirmed'].includes(appointment.status)) {
//       return responseError(res, `Không thể hủy lịch hẹn đang ở trạng thái '${appointment.status}'`, 400);
//     }
...
//     if (!appointment) return responseError(res, 'Không tìm thấy lịch hẹn', 404);
//     if (appointment.patient.toString() !== patientId.toString()) return responseError(res, 'Không có quyền xem lịch hẹn này', 403);

//     // Thêm datetimeVN để FE hiển thị
//     const appointmentData = appointment.toObject();
//     appointmentData.datetimeVN = toVN(appointment.datetime);
