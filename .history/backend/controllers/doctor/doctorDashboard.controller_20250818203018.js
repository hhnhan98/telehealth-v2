...
const { toVN, toUTC } = require('../../utils/timezoneUtils');
...
const getDashboardData = async (req, res) => {
  try {
    ...
    const todayStartVN = new Date(todayVN); todayStartVN.setHours(0,0,0,0);
    const todayEndVN = new Date(todayVN); todayEndVN.setHours(23,59,59,999);

    const todayAppointments = await Appointment.find({
      doctor: doctorId,
      datetime: { $gte: toUTC(todayStartVN), $lte: toUTC(todayEndVN) },
      status: { $ne: 'cancelled' }
    }).populate('patient', 'fullName email').sort({ datetime: 1 });

    const dayOfWeekVN = (todayStartVN.getDay() + 6) % 7;
    const mondayVN = new Date(todayStartVN); mondayVN.setDate(todayStartVN.getDate() - dayOfWeekVN); mondayVN.setHours(0,0,0,0);
    const sundayVN = new Date(mondayVN); sundayVN.setDate(mondayVN.getDate()+6); sundayVN.setHours(23,59,59,999);

    const weeklyAppointmentsCount = await Appointment.countDocuments({
      doctor: doctorId,
      datetime: { $gte: toUTC(mondayVN), $lte: toUTC(sundayVN) },
      status: { $ne: 'cancelled' }
    });
