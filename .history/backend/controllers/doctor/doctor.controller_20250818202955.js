...
const getWorkSchedule = async (req, res) => {
  try {
    ...
    if (view === 'week') {
      ...
      const dayOfWeekVN = (todayVN.getDay() + 6) % 7;
      startVN = new Date(todayVN); startVN.setDate(todayVN.getDate() - dayOfWeekVN); startVN.setHours(0,0,0,0);
      endVN = new Date(startVN); endVN.setDate(startVN.getDate()+6); endVN.setHours(23,59,59,999);
    } else {
      const todayVN = toVN(nowUTC);
      startVN = new Date(todayVN); startVN.setHours(0,0,0,0);
      endVN = new Date(todayVN); endVN.setHours(23,59,59,999);
    }
    ...
    for (let i = 0; i < numDays; i++) {
      ...
      const dayKey = dayVN.toISOString().split('T')[0];
      const slots = generateTimeSlots().map(time => ({ time, appointment: null }));

      appointments.filter(appt => toVN(appt.datetime).toISOString().split('T')[0] === dayKey)
        .forEach(appt => {
    ...
    return success(res, 'Lịch làm việc', schedule);
