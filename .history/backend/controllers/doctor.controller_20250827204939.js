...
//     })
//       .populate('patient', 'fullName email phone avatar birthYear')
//       .populate('location', 'name')
//       .populate('specialty', 'name')
//       .sort({ datetime: 1 });
...
//       .populate({
//         path: 'doctor',
//         populate: { path: 'user', select: 'fullName birthYear phone avatar email'}
//       })
//       .populate('location', 'name')
//       .populate('specialty', 'name');
...
const getDoctorAppointments = async (req, res) => {
  try {
    // Tìm doctor dựa vào user đăng nhập
    const doctor = await Doctor.findOne({ user: req.user._id });
    if (!doctor) return responseError(res, 'Không tìm thấy doctor', 404);

    // Lấy danh sách appointment của doctor, trạng thái confirmed/completed
    let appointments = await Appointment.find({
      ...
      .populate({
        ...
        populate: {
          path: 'user',
          select: 'fullName birthYear phone avatar email',
    ...
    console.log('>>> Raw appointments:', JSON.stringify(appointments, null, 2));

    // Convert sang object có virtuals
    const appointmentsObj = appointments.map(a => a.toObject({ virtuals: true }));

    console.log('>>> Appointments after toObject({virtuals:true}):', JSON.stringify(appointmentsObj, null, 2));

    return responseSuccess(res, 'Danh sách lịch hẹn', {
      count: appointmentsObj.length,
      appointments: appointmentsObj
    });
  } catch (err) {
    console.error('Lỗi getDoctorAppointments:', err);
    return responseError(res, 'Lỗi khi lấy danh sách lịch hẹn', 500);
  }
};
...
const getAppointmentDetail = async (req, res) => {
  try {
    ...
    console.log('>>> Appointment after toObject({virtuals:true}):', JSON.stringify(appointmentObj, null, 2));

    return responseSuccess(res, 'Chi tiết lịch hẹn', appointmentObj);
  } catch (err) {
    console.error('Lỗi getAppointmentDetail:', err);
    return responseError(res, 'Lỗi khi lấy chi tiết lịch hẹn', 500);
  }
};
...
// ===== Cải tiến dùng query aggregation, không cần map hay populate vòng lặp =====
const getMyPatients = async (req, res) => {
  try {
    // Tìm doctor theo user hiện tại
    const doctor = await Doctor.findOne({ user: req.user._id });
    ...
    // Lấy danh sách bệnh nhân đã có ít nhất 1 lịch completed với doctor
    const patients = await Patient.aggregate([
      // Lookup từ Appointment
      {
        $lookup: {
          from: 'appointments',         // collection trong MongoDB
          localField: 'user',           // Patient.user
          foreignField: 'patient',      // Appointment.patient
          as: 'appointments'
        }
      },
      // Chỉ giữ patient có ít nhất 1 appointment completed với doctor này
      {
        $match: {
          'appointments': {
            $elemMatch: { doctor: doctor._id, status: 'completed' }
          }
        }
      },
    ...
    return responseSuccess(res, 'Danh sách bệnh nhân', patients);
