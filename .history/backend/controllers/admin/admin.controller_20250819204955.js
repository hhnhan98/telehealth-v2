...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    if (role === 'doctor') {
      ...
      // Kiểm tra tồn tại specialty & location
      const foundSpecialty = await Specialty.findById(specialty);
      const foundLocation = await Location.findById(location);
      if (!foundSpecialty || !foundLocation) {
        await session.abortTransaction();
        session.endSession();
        return res.status(400).json({ success: false, message: 'Specialty hoặc Location không tồn tại' });
      }
    }
    ...
    if (role === 'doctor') {
      newDoctor = await Doctor.create(
        [{ user: newUser._id, specialty, location }],
        { session }
      );
    }
