...
exports.updateUser = async (req, res) => {
  ...
  try {
    ...
    if (user.role === 'doctor' && (update.specialty || update.location)) {
      ...
      if (doctor) {
        // Remove doctor từ Specialty/Location cũ
        ...
        await Doctor.findByIdAndUpdate(doctor._id, { specialty: update.specialty || doctor.specialty, location: update.location || doctor.location }, { session });
      }
    }
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    // Nếu là bác sĩ, remove khỏi Specialty/Location và xóa doctor
    if (user.role === 'doctor') {
      const doctor = await Doctor.findOne({ user: id }).session(session);
      if (doctor) {
