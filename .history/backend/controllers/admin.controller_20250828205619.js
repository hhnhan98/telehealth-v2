...
exports.createUser = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({
      success: true,
      data: { user: newUser, doctor: populatedDoctor },
    });
  } catch (err) {
...
exports.getAllLocations = async (req, res) => {
  try {
    const locations = await Location.find().populate('doctors', 'user specialty');
...
exports.getAllSpecialties = async (req, res) => {
  try {
    const specialties = await Specialty.find().populate('location', 'name').populate('doctors', 'user location');
...
exports.createSpecialty = async (req, res) => {
  try {
    ...
    // Kiểm tra trùng tên trong cùng cơ sở
    ...
    res.status(201).json({ success: true, data: await specialty.populate('location', 'name') });
  } catch (err) {
