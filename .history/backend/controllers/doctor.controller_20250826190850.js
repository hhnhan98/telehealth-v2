...
const getAppointmentsByDate = async (req, res) => {
  try {
    ...
    if (!date) return responseError(res, 'Thiếu ngày', 400);

    // Kiểm tra req.user._id trước khi truy vấn
    if (!req.user || !req.user._id) {
      console.error('>>> req.user hoặc req.user._id undefined');
      return responseError(res, 'Không xác định được user', 401);
    }
    ...
    // Xác định khoảng thời gian ngày chọn
    ...
    return responseSuccess(res, 'Lịch hẹn theo ngày', { appointments });
  } catch (err) {
    console.error('>>> Lỗi getAppointmentsByDate:', err);
    return responseError(res, 'Lỗi khi lấy lịch hẹn theo ngày', 500, err);
  }
};
