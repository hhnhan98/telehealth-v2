...
const getMedicalRecordsByPatient = async (req, res) => {
  try {
    ...
    // Map snapshot để trả về tên đầy đủ
    const recordsWithNames = records.map(r => ({
      ...r,
      doctorName: r.doctorSnapshot?.fullName || '-',
      specialtyName: r.doctorSnapshot?.specialtyName || r.doctorSnapshot?.specialty || '-',
      locationName: r.doctorSnapshot?.locationName || r.doctorSnapshot?.location || '-',
    }));
