...
exports.updateDoctor = async (req, res) => {
  ...
  try {
    ...
    // Nếu update location
    if (location) {
      // Reset specialty nếu không thuộc location mới
      if (doctor.specialty) {
        const currentSpec = await Specialty.findById(doctor.specialty).session(session);
        if (currentSpec && currentSpec.location.toString() !== location) {
