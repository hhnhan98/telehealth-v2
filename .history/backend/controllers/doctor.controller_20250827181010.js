...
const getDoctorAppointments = async (req, res) => {
  try {
    ...
    // Lấy tất cả lịch hẹn confirmed hoặc completed
    const appointments = await Appointment.find({
      ...
      .populate({
        path: 'patient',
        select: 'address user', // lấy address + ref user
        populate: {
          path: 'user',
      ...
      .populate('specialty', 'name')
      .sort({ datetime: 1 })
      .lean(); // dùng lean để trả về plain object, dễ map thêm

    // Map lại để FE dễ dùng: đặt fullName, birthYear, phone, avatar, email, address cùng cấp patient
    const formattedAppointments = appointments.map(a => ({
      ...a,
      patient: {
        _id: a.patient._id,
        fullName: a.patient.user?.fullName || '',
        birthYear: a.patient.user?.birthYear || null,
    ...
    return responseSuccess(res, 'Danh sách lịch hẹn', { count: formattedAppointments.length, appointments: formattedAppointments });
  } catch (err) {
