...
const updateProfile = async (req, res) => {
  try {
    ...
    if (birthYear !== undefined) userUpdates.birthYear = birthYear === '' ? null : Number(birthYear);
    ...
    if (Object.keys(userUpdates).length > 0 && doctor.user) {
      ...
      // 🔹 Log trước khi validate/save
      console.log('📌 User updates to save:', doctor.user);

      // Validate ngay trước khi save
      const validationError = doctor.user.validateSync();
      if (validationError) {
        console.error('❌ Validation error on User:', validationError);
        return responseError(res, 'Dữ liệu User không hợp lệ', 400, validationError);
      }
    ...
    // 🔹 Log Doctor trước khi save
    console.log('📌 Doctor updates to save:', doctor);
