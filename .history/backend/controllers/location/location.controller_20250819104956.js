...
const getAllLocations = async (req, res) => {
  try {
    ...
    console.responseError('Lỗi lấy danh sách địa điểm:', err);
    return responseError(res, 'Lỗi khi lấy danh sách địa điểm', 500);
  }
};
...
const createLocation = async (req, res) => {
  try {
    ...
    return re(res, 'Tạo địa điểm thành công', newLocation, 201);
  } catch (err) {
    console.responseError('Lỗi tạo địa điểm:', err);
    return responseError(res, 'Lỗi khi tạo địa điểm', 500);
  }
};
