...
const User = require('../../models/User'); // Bác sĩ cũng là user
...
const getDoctorsBySpecialtyAndLocation = async (req, res) => {
  ...
  if (!specialtyId) return res.status(400).json({ error: 'Thiếu specialtyId' });

  try {
    const doctors = await User.find({
      role: 'doctor',
      specialty: specialtyId
      // Nếu muốn filter location: location: locationId
    }).select('_id fullName email phone gender');
    res.json(doctors);
  } catch (err) {
...
const createAppointment = async (req, res) => {
  ...
  try {
    ...
    /* Gửi OTP qua email (tùy chỉnh nếu cần)
