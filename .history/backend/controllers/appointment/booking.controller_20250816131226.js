...
const createAppointment = async (req, res) => {
  try {
    ...
    // Sinh OTP (demo) và expiry 5 phút, có thể điều chỉnh
...
exports.getAppointments = async (req, res) => {
  try {
    const userId = req.user.id; // Lấy từ JWT (verifyToken gắn vào req.user)
    ...
    return res.status(500).json({ message: 'Không lấy được lịch hẹn' });
  }
};
