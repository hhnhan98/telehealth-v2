...
const getDoctorsBySpecialtyAndLocation = async (req, res) => {
  try {
    ...
    // Log nhận param từ FE
    console.log('Received query IDs -> Specialty:', specialtyId, '| Location:', locationId);
    ...
    doctors.forEach(doc => {
      console.log(`- Doctor: ${doc.user.fullName} | Specialty: ${doc.specialty?.name} | Location: ${doc.location?.name}`);
    });
