...
const createAppointment = async (req, res) => {
  try {
    const { doctorId, date, time, patientId, patient, reason } = req.body;
    ...
    if (patientId) {
      ...
      // Tạo patient mới cho guest
      const newPatient = await User.create({ ...patient, role: 'patient' });
      patientRef = newPatient._id;
    }

    // Tạo hoặc lấy schedule (giống logic cũ)
    let schedule = await Schedule.findOne({ doctorId, date });
    if (!schedule) {
    ...
    if (slotIndex === -1) return res.status(400).json({ error: 'Slot đã được đặt hoặc không tồn tại' });
