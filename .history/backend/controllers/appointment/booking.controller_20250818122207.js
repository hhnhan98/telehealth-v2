...
const getAvailableSlots = async (req, res) => {
  try {
    ...
    if (dayjs(date).isBefore(dayjs(), 'day')) {
      return res.status(400).json({ error: 'Không thể lấy slot quá khứ' });
    }

    // Tìm schedule
    let schedule = await Schedule.findOne({ doctorId, date });

    // Nếu chưa có schedule thì tạo mới
    if (!schedule) {
      ...
      if (dayOfWeek === 0) {
        ...
        slots = WORK_HOURS.slice(0, 7).map(t => ({ time: t, isBooked: false })); // Thứ 7 chỉ sáng
      } else {
        slots = WORK_HOURS.map(t => ({ time: t, isBooked: false })); // Ngày thường full
      }
