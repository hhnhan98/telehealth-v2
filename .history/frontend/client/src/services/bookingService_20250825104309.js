...
import { formatVN } from '../utils/timezone'; // formatVN sẽ convert UTC -> VN string
...
const fetchAvailableSlots = async (doctorId, date) => {
  ...
  try {
    ...
    const slots = Array.isArray(res?.data?.data?.availableSlots) ? res.data.data.availableSlots : [];

    // Map sang object { time, datetimeVN } để FE dùng trực tiếp
    return slots.map(slot => ({
      time: slot,             // giữ nguyên giá trị UTC hoặc string từ BE
      datetimeVN: formatVN(slot) // chuyển sang VN datetime string
    }));
  } catch (err) {
