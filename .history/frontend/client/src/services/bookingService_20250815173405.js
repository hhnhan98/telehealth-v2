...
// 3. Lấy danh sách bác sĩ theo specialty
export const fetchDoctors = async (specialtyId) => {
  if (!specialtyId) throw new Error('Thiếu specialtyId');
  const res = await axios.get(`/appointments/doctors?specialtyId=${specialtyId}`);
  return res.data;
};
