...
const AppointmentForm = ({ currentUser }) => {
  ...
  // ✅ Load Available Slots - Full Version
  const loadAvailableSlots = useCallback(async () => {
    if (!bookingData.doctor || !bookingData.date) {
      setAvailableTimes([]);
      return;
    ...
    try {
      ...
      // Extract slots đúng cấu trúc backend
      let slots = [];
      ...
      const normalizedSlots = (Array.isArray(slots) ? slots : []).map(slot => {
        const timeStr = typeof slot.time === 'string' ? slot.time : slot.time?.HHmm || '';

        // Format displayTime tránh Invalid Date
        ...
        if (slot.datetimeVN) {
          ...
          if (!isNaN(dt)) {
            ...
            displayTime = `${hours}:${minutes} ${day}/${month}/${year}`;
          }
        }

        return {
          time: timeStr,          // dùng làm value cho <option>
          displayTime,            // dùng hiển thị trong dropdown
  ...
  // ✅ useEffect gọi loadAvailableSlots khi doctor/date thay đổi
  useEffect(() => {
    setAvailableTimes([]);
    setBookingData(prev => ({ ...prev, time: '' }));
    loadAvailableSlots();
