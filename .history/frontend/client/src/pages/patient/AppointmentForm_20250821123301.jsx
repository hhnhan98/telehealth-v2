...
const AppointmentForm = ({ currentUser }) => {
  ...
  useEffect(() => {
    const loadLocations = async () => {
      try {
        ...
        showToast('Lỗi load cơ sở y tế', 'error');
      }
    };
    loadLocations();
  }, [showToast]);
  ...
  const loadSpecialties = useCallback(async () => {
    ...
    try {
      ...
      showToast('Lỗi load chuyên khoa', 'error');
      setSpecialties([]);
    }
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      ...
      showToast('Lỗi load bác sĩ', 'error');
      setDoctors([]);
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    if (!appointmentId) {
      showToast('Lỗi: Không tìm thấy thông tin appointment. Vui lòng đặt lại lịch.', 'error');
      return;
    }
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      showToast('OTP đã được gửi lại.', 'info');
    } catch (err) {
