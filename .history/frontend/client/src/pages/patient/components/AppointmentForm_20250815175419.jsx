...
const AppointmentForm = ({ currentUser }) => {
  ...
  useEffect(() => {
    fetchLocations()
      .then(setLocations)
      .catch(err => showToast('Lỗi load cơ sở y tế', 'error'));
  }, []);
  ...
  useEffect(() => {
    if (!bookingData.location) {
      setSpecialties([]);
      setDoctors([]);
      setBookingData(prev => ({ ...prev, specialty: '', doctor: '', time: '' }));
      return;
    }
    fetchSpecialties(bookingData.location)
      .then(setSpecialties)
      .catch(err => showToast('Lỗi load chuyên khoa', 'error'));
    setDoctors([]);
    setBookingData(prev => ({ ...prev, specialty: '', doctor: '', time: '' }));
  ...
  useEffect(() => {
    if (!bookingData.location || !bookingData.specialty) {
      setDoctors([]);
      setBookingData(prev => ({ ...prev, doctor: '', time: '' }));
      return;
    }
    fetchDoctors(bookingData.location, bookingData.specialty)
      .then(setDoctors)
      .catch(err => showToast('Lỗi load bác sĩ', 'error'));
    setBookingData(prev => ({ ...prev, doctor: '', time: '' }));
  }, [bookingData.location, bookingData.specialty]);

  // -------------------------
  // Load available slots khi chọn doctor + date
  // -------------------------
  useEffect(() => {
  ...
  // -------------------------
  // Prefill patient info nếu currentUser
  // -------------------------
  useEffect(() => {
    if (currentUser) {
      setBookingData(prev => ({
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      // Nếu currentUser đã đăng nhập, gửi patientId, còn không gửi object patient
      const payload = currentUser
        ? { ...bookingData, patientId: currentUser._id }
        : { ...bookingData };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || 'Xác thực OTP thất bại.';
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    }
