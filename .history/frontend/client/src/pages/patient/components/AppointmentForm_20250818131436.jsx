...
const AppointmentForm = ({ currentUser }) => {
  ...
  // Prefill patient info if user logged in
  useEffect(() => {
    if (currentUser) {
      setBookingData(prev => ({
        ...prev,
  ...
  // Load locations
  useEffect(() => {
    fetchLocations().then(res => setLocations(res.locations || [])).catch(() => showToast('Lỗi load cơ sở y tế', 'error'));
  }, []);
  ...
  // OTP & resend cooldown timers
  useEffect(() => { if (otpTimer <= 0) return; const interval = setInterval(() => setOtpTimer(prev => Math.max(prev-1,0)),1000); return () => clearInterval(interval); }, [otpTimer]);
  useEffect(() => { if (resendCooldown <= 0) return; const interval = setInterval(() => setResendCooldown(prev => Math.max(prev-1,0)),1000); return () => clearInterval(interval); }, [resendCooldown]);
  ...
  const validateForm = () => {
    ...
    if (!currentUser && (!patient.fullName || !patient.gender || !patient.dob)) { showToast('Vui lòng điền đầy đủ thông tin bệnh nhân.', 'error'); return false; }
    return true;
  };
  ...
  const res = await createAppointment(payload);
  console.log('>>> Response từ backend:', res);        // <-- Thêm dòng này
  ...
  console.error('>>> Booking error detail:', err.response?.data || err.message || err); // <-- Thêm dòng này
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      setBookingData(prev => ({
        ...
        patient: currentUser ? { ...prev.patient, reason: '' } : { fullName:'', gender:'', dob:'', phone:'', email:'', reason:'' }
      }));
  ...
  const handleResendOtp = async () => {
    ...
    try {
      await resendOtp(createdAppointmentId);
      ...
      {!otpStage ? (
        <form className="appointment-form-grid" onSubmit={handleSubmit}>
          ...
          <div className="form-section">
            ...
            <div className="form-group"><input type="text" name="fullName" placeholder="Họ tên" value={bookingData.patient.fullName} onChange={handlePatientChange} required={!currentUser} /></div>
            <div className="form-group">
              <select name="gender" value={bookingData.patient.gender} onChange={handlePatientChange} required={!currentUser}>
                <option value="">Giới tính</option>
                <option value="male">Nam</option>
                <option value="female">Nữ</option>
              </select>
            </div>
            <div className="form-group"><input type="date" name="dob" value={bookingData.patient.dob} onChange={handlePatientChange} required={!currentUser} /></div>
            <div className="form-group"><input type="text" name="phone" placeholder="Số điện thoại" value={bookingData.patient.phone} onChange={handlePatientChange} /></div>
            <div className="form-group"><input type="email" name="email" placeholder="Email" value={bookingData.patient.email} onChange={handlePatientChange} /></div>
            <div className="form-group full-width"><textarea name="reason" placeholder="Lý do khám" value={bookingData.patient.reason} onChange={handlePatientChange} /></div>
          </div>

          <button type="submit" disabled={loadingSubmit}>{loadingSubmit ? 'Đang gửi...' : 'Đặt lịch'}</button>
        </form>
      ) : (
