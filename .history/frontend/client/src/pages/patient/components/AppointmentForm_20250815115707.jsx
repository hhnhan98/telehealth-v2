...
const AppointmentForm = ({ currentUser }) => {
  ...
  // Load locations
  useEffect(() => {
    axios.get('/appointment/locations')
  ...
  useEffect(() => {
    if (bookingData.location) {
      axios.get(`/appointment/specialties?locationId=${bookingData.location}`)
  ...
  useEffect(() => {
    if (bookingData.location && bookingData.specialty) {
      axios.get(`/appointment/doctors?locationId=${bookingData.location}&specialtyId=${bookingData.specialty}`)
  ...
  useEffect(() => {
    if (bookingData.doctor && bookingData.date) {
      axios.get(`/appointment/available-slots?doctorId=${bookingData.doctor}&date=${bookingData.date}`)
  ...
  const handleSubmit = async e => {
    ...
    try {
      const { location, specialty, doctor, date, time, patient } = bookingData;
      const res = await axios.post('/appointment/create', { location, specialty, doctor, date, time, patient });
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoading(true);
    try {
      await axios.post('/appointment/verify-otp', { appointmentId: createdAppointmentId, otp: otpInput });
