...
const AppointmentForm = ({ currentUser }) => {
  ...
  const loadSpecialties = useCallback(async () => {
    ...
    try {
      const res = await bookingService.fetchSpecialties(bookingData.location, { signal: controller.signal });
      const list = Array.isArray(res.specialties) ? res.specialties : [];
      setSpecialties(list);
      specialtiesCache.current[bookingData.location] = list;
    } catch (err) {
      if (err.name !== 'AbortError') showToast('Lỗi load chuyên khoa');
    }
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      const res = await bookingService.fetchDoctors(bookingData.location, bookingData.specialty, { signal: controller.signal });
      const list = Array.isArray(res.doctors) ? res.doctors : [];
      setDoctors(list);
      doctorsCache.current[key] = list;
    } catch (err) {
      if (err.name !== 'AbortError') showToast('Lỗi load bác sĩ');
    }
  ...
  const loadAvailableSlots = useCallback(async () => {
    ...
    try {
      const res = await bookingService.fetchAvailableSlots(bookingData.doctor, bookingData.date, { signal: controller.signal });
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      console.error(err);
      showToast(err.response?.data?.error || 'Đặt lịch thất bại.');
    } finally {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast(err.response?.data?.error || 'Xác thực OTP thất bại.');
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      console.error(err);
      showToast(err.response?.data?.error || 'Gửi lại OTP thất bại');
    }
  };
