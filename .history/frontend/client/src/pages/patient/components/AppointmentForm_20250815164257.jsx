...
const AppointmentForm = ({ currentUser }) => {
  ...
  // -------------------------
  useEffect(() => {
    fetchLocations().then(setLocations).catch(err => showToast('Lỗi load locations', 'error'));
  }, []);
  ...
  useEffect(() => {
    ...
    fetchSpecialties(bookingData.location)
      .then(setSpecialties)
      .catch(err => showToast('Lỗi load specialties', 'error'));
    setBookingData(prev => ({ ...prev, specialty: '', doctor: '' }));
    setDoctors([]);
  ...
  useEffect(() => {
    ...
    fetchDoctors(bookingData.location, bookingData.specialty)
      .then(setDoctors)
      .catch(err => showToast('Lỗi load doctors', 'error'));
    setBookingData(prev => ({ ...prev, doctor: '' }));
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      showToast(err.error || 'Đặt lịch thất bại.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      showToast(err.error || 'Xác thực OTP thất bại.', 'error');
    } finally {
      setLoading(false);
    }
  };
