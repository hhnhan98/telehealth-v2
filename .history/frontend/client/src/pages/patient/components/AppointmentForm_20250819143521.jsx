...
const AppointmentForm = () => {
  ...
  useEffect(() => {
    const loadSpecialties = async () => {
      ...
      try {
        const res = await fetchSpecialties(bookingData.location, { signal: controller.signal });
        const list = res.data || [];
        setSpecialties(list);
        specialtiesCache.current[bookingData.location] = list;
      } catch (err) {
        if (err.name !== 'AbortError') alert('Lỗi load chuyên khoa');
      }
    };
  ...
  useEffect(() => {
    const loadDoctors = async () => {
      ...
      try {
        const res = await fetchDoctors(bookingData.location, bookingData.specialty, { signal: controller.signal });
        const list = res.data || [];
        setDoctors(list);
        doctorsCache.current[key] = list;
      } catch (err) {
        if (err.name !== 'AbortError') alert('Lỗi load bác sĩ');
      }
    };
  ...
  useEffect(() => {
    const loadSlots = async () => {
      ...
      try {
        const res = await fetchAvailableSlots(bookingData.doctor, bookingData.date, { signal: controller.signal });
        setAvailableTimes(res.data || []);
      } catch (err) {
        if (err.name !== 'AbortError') alert('Lỗi load lịch khám');
      }
    };
    loadSlots();
  ...
  const validateForm = () => {
    ...
    if (availableTimes.length === 0) {
      alert('Không còn slot trống cho ngày này.');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setCreatedAppointmentId(res.data._id);
      setOtpStage(true);
      setOtpTimer(300);
      setResendCooldown(30);
      alert('Đặt lịch thành công! OTP đã gửi email.');
    } catch (err) {
  ...
  const handleVerifyOtp = async () => {
    if (!otpInput) return alert('Vui lòng nhập OTP');
    try {
  ...
  const handleResendOtp = async () => {
    ...
    try {
      ...
      alert('OTP đã gửi lại.');
    } catch (err) {
      console.error('Resend OTP error:', err);
      alert(err.message || 'Gửi lại OTP thất bại');
    }
  };
      ...
      {!otpStage ? (
        ...
        <div className="otp-verification card">
          ...
          <div className="otp-actions">
            ...
            <button onClick={handleResendOtp} disabled={resendCooldown > 0} className={resendCooldown > 0 ? 'disabled' : ''}>
