...
const AppointmentForm = ({ currentUser }) => {
  ...
  const loadSpecialties = useCallback(async (locationId) => {
    ...
    try {
      const specs = await specialtyService.fetchSpecialtiesByLocation(locationId);
      setSpecialties(Array.isArray(specs) ? specs : []);
      specialtiesCache.current[locationId] = specs;
    } catch (err) {
      console.error('Load specialties error:', err);
      alert('Lỗi load chuyên khoa.');
    }
  }, []);
  ...
  // ---------------- Load Doctors ----------------
  const loadDoctors = useCallback(async (locationId, specialtyId) => {
    if (!locationId || !specialtyId) return setDoctors([]);
    const key = `${locationId}_${specialtyId}`;
    if (doctorsCache.current[key]) {
      setDoctors(doctorsCache.current[key]);
      return;
    }
    try {
      const res = await bookingService.fetchDoctors(locationId, specialtyId);
      const list = Array.isArray(res) ? res : res.doctors || [];
      setDoctors(list);
      doctorsCache.current[key] = list;
    } catch (err) {
      console.error('Load doctors error:', err);
      alert('Lỗi load bác sĩ.');
    }
  }, []);
  ...
  useEffect(() => {
    ...
    loadDoctors(bookingData.location, bookingData.specialty);
  ...
  const loadAvailableSlots = useCallback(async () => {
    ...
    try {
      ...
      setAvailableTimes(Array.isArray(res) ? res : res.availableSlots || []);
    } catch (err) {
      ...
      {!otpStage ? (
        <form className="appointment-form-grid" onSubmit={handleSubmit}>
          ...
          <div className="form-section card">
            ...
            <textarea name="reason" value={bookingData.patient.reason} onChange={handlePatientChange} placeholder="Nhập triệu chứng..." />
          </div>
