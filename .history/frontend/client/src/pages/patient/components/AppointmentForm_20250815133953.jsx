...
const AppointmentForm = ({ currentUser }) => {
  ...
  // Load locations
  useEffect(() => {
    axios.get('/api/appointments/locations')
  ...
  useEffect(() => {
    if (bookingData.location) {
      axios.get(`/api/appointments/specialties?locationId=${bookingData.location}`)
  ...
  // Load doctors when location & specialty change
  useEffect(() => {
    if (bookingData.location && bookingData.specialty) {
      axios.get(`/api/appointments/doctors?locationId=${bookingData.location}&specialtyId=${bookingData.specialty}`)
  ...
  useEffect(() => {
    if (bookingData.doctor && bookingData.date) {
      axios.get(`/api/appointments/available-slots?doctorId=${bookingData.doctor}&date=${bookingData.date}`)
  ...
  // Prefill patient info from currentUser
  useEffect(() => {
    if (currentUser) {
      setBookingData(prev => ({
        ...prev,
  ...
  const handleSubmit = async e => {
    ...
    try {
      const { location, specialty, doctor, date, time, patient } = bookingData;
      const res = await axios.post('/api/appointments/create', { location, specialty, doctor, date, time, patient });
      setToast({ message: 'Đặt lịch thành công! Vui lòng nhập OTP được gửi qua email.', type: 'success' });
  ...
  const handleVerifyOtp = async () => {
    if (!otpInput) return setToast({ message: 'Vui lòng nhập OTP', type: 'error' });
    setLoading(true);
    try {
      await axios.post('/api/appointments/verify-otp', { appointmentId: createdAppointmentId, otp: otpInput });
      setToast({ message: 'Xác thực OTP thành công. Lịch hẹn đã được xác nhận.', type: 'success' });
      setOtpStage(false);
      setOtpInput('');
