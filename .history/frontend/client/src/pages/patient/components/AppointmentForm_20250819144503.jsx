...
const AppointmentForm = ({ currentUser }) => {
  ...
  const specialtiesCache = useRef({}); // cache theo locationId
  const doctorsCache = useRef({});     // cache theo locationId_specialtyId
  ...
  const loadSpecialties = useCallback(async (locationId) => {
    ...
    try {
      const res = await specialtyService.fetchSpecialtiesByLocation(locationId, { signal: controller.signal });
      setSpecialties(res || []);
      specialtiesCache.current[locationId] = res || [];
    } catch (err) {
  ...
  useEffect(() => {
    ...
    setDoctors([]);
    loadSpecialties(bookingData.location);
  ...
  const loadDoctors = useCallback(async () => {
    ...
    try {
      const res = await userService.fetchDoctorsBySpecialty(bookingData.location, bookingData.specialty, { signal: controller.signal });
      setDoctors(res || []);
      doctorsCache.current[key] = res || [];
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const loadSlots = async () => {
      try {
        const res = await userService.fetchAvailableSlots(bookingData.doctor, bookingData.date, { signal: controller.signal });
  ...
  const validateForm = () => {
    ...
    if (!patient.reason) {
      showToast('Vui lòng nhập lý do khám', 'error');
      return false;
    }
    if (availableTimes.length === 0) {
      showToast('Không còn slot trống cho ngày này', 'error');
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      const res = await userService.createAppointment({
      ...
      setCreatedAppointmentId(res._id);
      setOtpStage(true);
      setOtpTimer(300);
  ...
  const handleVerifyOtp = async () => {
    ...
    setLoadingSubmit(true);
    try {
      await userService.verifyOtp(createdAppointmentId, otpInput);
      ...
      showToast('Xác thực OTP thất bại', 'error');
    } finally {
      setLoadingSubmit(false);
    }
  };
  ...
  const handleResendOtp = async () => {
    ...
    setOtpTimer(300);
    try {
      await userService.resendOtp(createdAppointmentId);
      showToast('OTP đã gửi lại', 'success');
    } catch (err) {
    ...
    <div className="appointment-form-container">
      ...
      {!otpStage ? (
        ...
        <div className="otp-verification card">
          ...
          <button onClick={handleVerifyOtp}>{loadingSubmit ? 'Đang xác thực...' : 'Xác nhận OTP'}</button>
          <button onClick={handleResendOtp} disabled={resendCooldown>0}>{resendCooldown>0 ? `Gửi lại sau ${resendCooldown}s` : 'Gửi lại OTP'}</button>
          {otpTimer>0 && <p>OTP còn hiệu lực: {formatTimer(otpTimer)}</p>}
        </div>
      )}
    </div>
  );
