...
const AppointmentForm = ({ currentUser }) => {
  ...
  useEffect(() => {
    const loadLocations = async () => {
      try {
        const list = await bookingService.fetchLocations(); // trả về array trực tiếp
        setLocations(list);
      } catch (err) {
        console.error(err);
  ...
  const loadAvailableSlots = useCallback(async () => {
    ...
    try {
      const list = await bookingService.fetchAvailableSlots(bookingData.doctor, bookingData.date);
