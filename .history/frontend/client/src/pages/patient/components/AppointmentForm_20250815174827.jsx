...
const AppointmentForm = ({ currentUser }) => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (!res || !res.appointment) {
        throw new Error(res?.message || 'Đặt lịch thất bại, không nhận được dữ liệu.');
      }
      ...
      const msg = err.response?.data?.error || err.message || 'Đặt lịch thất bại.';
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.error || err.message || 'Xác thực OTP thất bại.';
      showToast(msg, 'error');
    } finally {
      setLoading(false);
    }
