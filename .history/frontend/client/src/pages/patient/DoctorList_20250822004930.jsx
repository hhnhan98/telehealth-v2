...
const DoctorList = () => {
  ...
  useEffect(() => {
    const fetchDoctors = async () => {
      try {
        ...
        // Fix dữ liệu: lấy đúng mảng doctors
  ...
  const filteredDoctors = doctors.filter(doc => {
    ...
    if (filters.name.trim() && !doc.fullName?.toLowerCase().includes(filters.name.toLowerCase())) return false;
    return true;
  });
    ...
    <div style={{ maxWidth: '1000px', margin: '20px auto', padding: '0 15px' }}>
      ...
      <div style={{
        ...
        {filteredDoctors.map(doc => (
          ...
          >
            <img
              src={doc.avatar || 'https://via.placeholder.com/120'}
              alt={doc.userfullName || ''}
              width={120}
              height={120}
