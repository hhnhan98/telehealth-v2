...
function PatientLayout() {
  ...
  const loadUser = useCallback(async () => {
    try {
      ...
      navigate('/login'); // redirect nếu fetch user thất bại
    } finally {
  ...
  const sidebarLinks = [
    ...
    { label: 'Đặt lịch hẹn', path: '/patient/book-appointment' },
    { label: 'Lịch sử', path: '/patient/appointments' },
    { label: 'Hồ sơ cá nhân', path: '/patient/profile' },
  ];
    ...
    <div style={styles.wrapper}>
      ...
      <aside style={styles.sidebar}>
        ...
        {sidebarLinks.map((link) => (
          <button
            key={link.path}
            style={location.pathname.includes(link.path) ? styles.activeBtn : styles.btn}
      ...
      <main style={styles.main}>
        {location.pathname.includes('/profile') ? (
          ...
          <Outlet context={{ currentUser, reloadUser: loadUser }} />
        )}
      </main>
    </div>
  );
...
//       setLoadingUser(true);
//       const res = await userService.fetchUserProfile();
//       setCurrentUser(res || null); // fetchUserProfile đã trả về data trực tiếp
//     } catch (err) {
//       console.error('Lỗi khi tải user:', err);
//     } finally {
//       setLoadingUser(false);
