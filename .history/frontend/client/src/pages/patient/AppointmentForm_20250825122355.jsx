...
const AppointmentForm = ({ currentUser }) => {
  ...
  const loadAvailableSlots = useCallback(async () => {
    ...
    try {
      ...
      // 🔹 Chuẩn hóa dữ liệu để dropdown hiển thị
      const normalizedSlots = (Array.isArray(slots) ? slots : []).map(s => ({
        time: s.time?.time || "",         // hiển thị giờ
        datetime: s.time?.datetimeVN || "" // gửi backend
      }));
