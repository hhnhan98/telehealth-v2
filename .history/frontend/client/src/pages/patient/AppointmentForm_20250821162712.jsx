...
const AppointmentForm = ({ currentUser }) => {
  ...
  const loadAvailableSlots = useCallback(async () => {
    ...
    try {
      ...
      setAvailableTimes(Array.isArray(res) ? res : []);
    } catch (err) {
      console.error(err);
      setAvailableTimes([]);
    }
  ...
  return (
    <div className="appointment-form-container">
      <h2 className="form-title text-primary">Đặt lịch khám bệnh</h2>

      {!otpStage ? (
        <form className="appointment-form-grid" onSubmit={handleSubmit}>
          <div className="form-section card">
            <h3 className="section-title text-primary-dark">Thông tin lịch khám</h3>
          ...
          <div className="form-section card">
            <h3 className="section-title text-primary-dark">Triệu chứng / Lý do khám</h3>
            <div className="form-group">
              <textarea name="reason" placeholder="Vui lòng nhập triệu chứng..." value={bookingData.patient.reason} onChange={handlePatientChange}/>
            </div>
          </div>

          <button type="submit" className="btn-submit btn-primary" disabled={loadingSubmit || availableTimes.length === 0}>
            {loadingSubmit ? 'Đang gửi...' : 'Đặt lịch'}
          </button>
        </form>
      ) : (
        <div className="otp-verification card">
          <h3 className="section-title text-primary-dark">Nhập mã OTP để xác nhận lịch hẹn</h3>
          <input type="text" value={otpInput} onChange={e => setOtpInput(e.target.value)} placeholder="Mã OTP" />
          <div className="otp-actions">
            <button type="button" className="btn-verify" onClick={handleVerifyOtp}>Xác nhận OTP</button>
            <button type="button" className="btn-resend" onClick={handleResendOtp} disabled={resendCooldown > 0}>
...
//     // console.log('DEBUG handleVerifyOtp:', { 
//     //   createdAppointmentId, 
//     //   appointmentIdFromStorage: localStorage.getItem('tempAppointmentId'),
