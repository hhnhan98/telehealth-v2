...
const DoctorList = () => {
  ...
  const specialties = Array.from(new Set(doctors.map(doc => doc.specialty?.name).filter(Boolean)));
  const locations = Array.from(new Set(doctors.map(doc => doc.location?.name).filter(Boolean)));
  ...
  useEffect(() => {
    ...
    if (filters.name.trim() !== '') {
      ...
      filtered = filtered.filter(doc => doc.user.fullName.toLowerCase().includes(keyword));
    }
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '20px auto', padding: '0 15px' }}>
      ...
      <div style={{ display: 'flex', gap: '15px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <select value={filters.specialty} onChange={e => setFilters(prev => ({ ...prev, specialty: e.target.value }))}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '15px' }}>
        {filteredDoctors.map(doc => (
          <div key={doc._id} style={{
            ...
            <p style={{ margin: '5px 0', fontWeight: 'bold' }}>{doc.specialty?.name || '-'}</p>
            <p style={{ margin: '5px 0', fontStyle: 'italic', color: '#555' }}>{doc.location?.name || '-'}</p>
            <p style={{ margin: '10px 0', fontSize: '0.9rem', color: '#333' }}>{doc.bio || 'Chưa có thông tin bio'}</p>
            <button
