...
// Load Available Slots chuẩn hóa với timezone VN
const loadAvailableSlots = useCallback(async () => {
  const { doctor, date } = bookingData;
  if (!doctor || !date) {
    setAvailableTimes([]);
    return;
  }
  ...
  try {
    const res = await bookingService.fetchAvailableSlots(doctor, date);
    const list = Array.isArray(res) ? res : [];

    // Chuẩn hóa datetime về ISO string VN
    const normalized = list
      .filter(slot => slot.datetime) // lọc bỏ slot không có datetime
      .map(slot => ({
        ...slot,
        datetime: formatVN(slot.datetime, 'YYYY-MM-DDTHH:mm:ss'), // VN timezone
        time: dayjs(slot.datetime).tz('Asia/Ho_Chi_Minh').format('HH:mm'),
      }));
...
useEffect(() => {
  ...
  // Gọi hàm loadAvailableSlots nếu đã chọn doctor và date
          ...
          <div className="form-section card">
            ...
            <div className="form-group">
              ...
              >
                ...
                {availableTimes
                  .filter(slot => slot.datetime && dayjs(slot.datetime).isValid())
                  .map((slot, index) => (
                    <option key={slot._id || index} value={slot.time}>
                      {formatVN(slot.datetime)}
                    </option>
                ))}
              </select>
            </div>
