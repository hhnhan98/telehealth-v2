...
const PatientMedicalRecordList = () => {
  ...
  const fetchRecords = useCallback(async (pageNumber = 1) => {
    ...
    setError('');
    try {
      const res = await patientService.getMyMedicalRecords({ page: pageNumber, limit });
      if (res.success) {
          ...
          {records.length === 0 ? (
            ...
            <table className="record-table">
              ...
              <tbody>
                {records.map(r => (
                  <tr key={r._id}>
                    <td>{r.appointment?.date ? dayjs(r.appointment.date).format('DD/MM/YYYY HH:mm') : '-'}</td>
                    <td>{r.doctorName || r.doctorSnapshot?.fullName || '-'}</td>
                    <td>{r.specialtyName || r.doctorSnapshot?.specialtyName || '-'}</td>
                    <td>{r.locationName || r.doctorSnapshot?.locationName || '-'}</td>
                    <td>
