...
const AppointmentForm = ({ currentUser }) => {
  ...
  // ✅ SỬA LẠI: Load Available Slots - LOGIC CHÍNH XÁC
  const loadAvailableSlots = useCallback(async () => {
    if (!bookingData.doctor || !bookingData.date) {
      setAvailableTimes([]);
      return;
    ...
    try {
      ...
      if (response?.data?.availableSlots) {
        // Nếu backend trả về { data: { availableSlots: [...] } }
        slots = response.data.availableSlots;
      } else if (Array.isArray(response)) {
        // Nếu service đã extract ra array
        slots = response;
      ...
      // ✅ CHUẨN HÓA DỮ LIỆU - LOGIC ĐÚNG
      const normalizedSlots = (Array.isArray(slots) ? slots : []).map(slot => ({
        time: slot.time,           // ✅ ĐÚNG: time string như "08:00"
        displayTime: slot.time,    // Để hiển thị trong dropdown
        datetimeVN: slot.datetimeVN // Để hiển thị thêm info nếu cần
      }));
          ...
          <div className="form-section card">
            ...
            <div className="form-group">
              ...
              >
                ...
                {availableTimes.map((slot, index) => (
                  <option key={index} value={slot.time}>
                    {slot.displayTime} {/* Hiển thị đúng chuỗi giờ VN */}
                  </option>
                ))}
              </select>
            </div>
