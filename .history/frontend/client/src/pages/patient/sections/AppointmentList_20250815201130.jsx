        ...
        <table className="appointment-table">
          ...
          <tbody>
            {appointments.map(appt => (
              <tr key={appt._id}>
                <td>{new Date(`${appt.date}T${appt.time}`).toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit', year: 'numeric' })}</td>
                <td>{appt.doctor?.fullName || 'Chưa có'}</td>
                <td>{appt.specialty?.name || 'Chưa có'}</td>
                <td>{appt.status}</td>
                <td>{appt.status === 'confirmed' ? <button onClick={() => handleCancel(appt._id)} className="cancel-btn">Hủy</button> : <span>-</span>}</td>
              </tr>
            ))}
          </tbody>
        </table>}
