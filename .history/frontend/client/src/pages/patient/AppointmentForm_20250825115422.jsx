...
//  Load Available Slots (chuẩn hóa timezone VN)
const loadAvailableSlots = useCallback(async () => {
  ...
  try {
    // Chuẩn hóa date VN trước khi gửi API (YYYY-MM-DD)
    const vnDate = dayjs(bookingData.date).format('YYYY-MM-DD');

    const res = await bookingService.fetchAvailableSlots(
      bookingData.doctor,
      vnDate
    );
    ...
    // Chuẩn hóa datetime: đảm bảo slot có datetime hợp lệ
    const normalized = list
      .map((slot) => {
        if (!slot.datetime) return null;
        const isoUTC = dayjs(slot.datetime).toISOString(); // UTC string
        return {
          ...slot,
          datetime: isoUTC,
          timeVN: formatVN(isoUTC, 'HH:mm'), // value cho dropdown
        };
      })
      .filter(Boolean);

    console.log('Available slots:', normalized); // Debug
