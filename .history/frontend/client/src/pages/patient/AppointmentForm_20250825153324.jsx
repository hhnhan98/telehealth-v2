    ...
    try {
      ...
      const normalized = slots.map(slot => {
        ...
        if (slot.datetimeVN) {
          ...
          if (!isNaN(dt)) displayTime = `${dt.getHours().toString().padStart(2,'0')}:${dt.getMinutes().toString().padStart(2,'0')}`;
        }
        return { time: slot.time, displayTime, datetimeVN: slot.datetimeVN };
      });
        ...
        <form className="appointment-form-grid" onSubmit={handleSubmit}>
          <div className="form-section card">
            ...
            <div className="form-group">
              ...
              <select 
                value={bookingData.time} onChange={e => setBookingData(prev => ({ ...prev, time: e.target.value }))} disabled={!bookingData.date || availableTimes.length === 0 || loadingSlots}>
