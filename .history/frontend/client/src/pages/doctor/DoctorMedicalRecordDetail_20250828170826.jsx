...
const DoctorMedicalRecordDetail = ({ record, onClose }) => {
  ...
  return (
    <Modal onClose={onClose} 
           title={`Chi tiết phiếu khám ${record.patientSnapshotpatient?.fullName || '-'}`}
           size="full"
           isOpen={!!record}
    >            
      <div className="medical-record-detail-modal">
        <div className="modal-section">
          ...
          <p><strong>Bệnh nhân:</strong> {record.patientSnapshot?.fullName || '-'}</p>
          <p><strong>Bác sĩ phụ trách:</strong> {record.doctorName || record.doctor?.fullName || '-'}</p>
          <p><strong>Chuyên khoa:</strong> {record.specialtyName || '-'}</p>
          <p><strong>Cơ sở:</strong> {record.locationName || '-'}</p>
          <p><strong>Thời gian khám:</strong> {formatDateTime(record.appointment?.date, record.appointment?.time)}</p>
        </div>
