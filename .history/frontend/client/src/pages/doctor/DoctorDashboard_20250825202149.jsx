...
const DoctorDashboard = () => {
  const [dashboard, setDashboard] = useState({
    todayAppointments: [],
    weeklySlots: [] // [{ date: '2025-08-25', slots: [{time: '08:00', booked: true, patient: 'Nguyen Van A'}]}]
  });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const navigate = useNavigate();
  ...
  const { todayAppointments, weeklySlots } = dashboard;

  const formatDate = (dateStr) => {
    const date = new Date(dateStr);
    return `${['CN','T2','T3','T4','T5','T6','T7'][date.getDay()]} - ${date.getDate()}/${date.getMonth()+1}`;
  };
            ...
            <div key={hour} className="calendar-row">
              {weeklySlots.map((day) => {
                ...
                return (
                  <div
                    key={day.date + hour}
                    className={`calendar-slot ${slot?.booked ? 'booked' : 'free'}`}
