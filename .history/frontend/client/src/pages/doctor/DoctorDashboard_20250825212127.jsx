...
import { useNavigate } from 'react-router-dom';
import doctorService from '../../services/doctorService';
import './styles/DoctorDashboard1css';
...
const DoctorDashboard = () => {
  ...
  // Lấy danh sách slot cho ngày được chọn
  const getSlotsForDate = () => {
    // Kiểm tra slot đã được đặt
    return SLOT_TIMES.map(time => {
      const appt = dashboard.todayAppointments.find(a => {
        const apptDate = new Date(a.datetime);
        return apptDate.toDateString() === selectedDate.toDateString() && apptDate.getHours() === parseInt(time.split(':')[0]) && apptDate.getMinutes() === parseInt(time.split(':')[1]);
      });
