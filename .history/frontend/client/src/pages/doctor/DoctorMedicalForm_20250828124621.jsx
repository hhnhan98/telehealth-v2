...
import { createMedicalReceipt } from '../../services/appointmentService';
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const bmi = form.height && form.weight
        ...
        : '';
      const payload = { appointmentId: id, ...form, bmi };

      const res = await createMedicalReceipt(id, payload);

      toast.success('Phiếu khám đã được lưu!');

      // Gửi record mới về parent DoctorMedicalRecordList để tự động cập nhật
      if (onRecordSaved) {
        onRecordSaved(res.data.data);
      }
