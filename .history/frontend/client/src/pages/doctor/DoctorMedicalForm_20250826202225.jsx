          ...
          <Card className="section-card">
            ...
            {form.prescriptions.map((p, idx) => (
              <div key={idx} className="prescription-row">
                <input placeholder="Tên thuốc" value={p.name} onChange={e => handlePrescriptionChange(idx,'name',e.target.value)} className={formErrors[`prescription-name-${idx}`] ? 'error-input' : ''}/>
                <input placeholder="Liều lượng" value={p.dose} onChange={e => handlePrescriptionChange(idx,'dose',e.target.value)} className={formErrors[`prescription-dose-${idx}`] ? 'error-input' : ''}/>
                <input type="number" min={1} placeholder="Số lượng" value={p.quantity} onChange={e => handlePrescriptionChange(idx,'quantity',e.target.value)} className={formErrors[`prescription-quantity-${idx}`] ? 'error-input' : ''}/>
                <input placeholder="Đơn vị" value={p.unit} onChange={e => handlePrescriptionChange(idx,'unit',e.target.value)}/>
                <input placeholder="Ghi chú" value={p.note} onChange={e => handlePrescriptionChange(idx,'note',e.target.value)}/>
