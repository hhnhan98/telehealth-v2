...
const DoctorDashboard = () => {
  ...
  const loadDashboard = useCallback(async () => {
    try {
      ...
      console.log('[Dashboard API] res:', res);
  ...
  const loadAppointmentsByDate = useCallback(async (date) => {
    try {
      ...
      console.log('[Appointments API] loading for date:', formatted);
      ...
      console.error('[Appointments API] Lỗi loadAppointmentsByDate:', err);
      setAppointmentsByDate([]);
    }
  }, []);

  useEffect(() => { loadAppointmentsByDate(selectedDate); }, [selectedDate, loadAppointmentsByDate]);
  ...
  const getSlotsForDate = () => {
    ...
    return SLOT_TIMES.map(time => {
      ...
      return {
        time,
        booked: !!appt,
        patient: appt?.patient?.user?.fullName || 'Không rõ',
        id: appt?._id || null,
      };
    });
  };
