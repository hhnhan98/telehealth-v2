...
const DoctorDashboard = () => {
  ...
  // ===== Load appointments cho ngày chọn =====
  const loadAppointmentsByDate = useCallback(async (date) => {
    try {
      if (!date) return setAppointmentsByDate([]);
      const res = await doctorService.fetchAppointmentsByDate(date.toISOString().split('T')[0]);
  ...
  const handleClickSlot = (slot) => {
    if (slot.booked && slot.id) {
      navigate(`/doctor/appointments/${slot.id}`);
    }
  };
  ...
  const getSlotsForDate = () => {
    ...
    return SLOT_TIMES.map(time => {
      const appt = appointmentsByDate.find(a => {
        ...
        return apptDate.getHours() === parseInt(time.split(':')[0], 10) &&
