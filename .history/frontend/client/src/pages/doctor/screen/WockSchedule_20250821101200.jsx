...
const DoctorSchedule = () => {
  ...
  const handleUpdateStatus = async (status) => {
    if (selectedAppointment) {
      try {
        ...
        loadSchedule(); // Tải lại lịch sau khi cập nhật
        closeModal();
      } catch (err) {
        console.error(err);
  ...
  const handleCancelAppointment = async () => {
    if (selectedAppointment) {
      try {
        await scheduleService.cancelAppointment(selectedAppointment._id);
        loadSchedule(); // Tải lại lịch sau khi hủy
        closeModal();
      } catch (err) {
        console.error(err);
        ...
        <div className="modal">
          <div className="modal-content">
            ...
            <div className="modal-actions">
              <button onClick={() => handleUpdateStatus('confirmed')}>Xác nhận</button>
              <button onClick={() => handleUpdateStatus('cancelled')}>Hủy</button>
              <button onClick={handleCancelAppointment}>Hủy lịch hẹn</button>
            </div>
          </div>
        </div>
      )}
