...
const MedicalRecordForm = ({ patientId, record, onSuccess, onCancel }) => {
  ...
  useEffect(() => {
    if (record) {
      ...
      setVisitDate(record.visitDate ? record.visitDate.slice(0, 10) : ''); // format yyyy-mm-dd
    } else {
      setSymptoms('');
      setDiagnosis('');
      setPrescriptions('');
  ...
  return (
    <form onSubmit={handleSubmit} className="medical-record-form">
      ...
      <div className="form-group">
        ...
        <textarea value={prescriptions} onChange={(e) => setPrescriptions(e.target.value)} rows={3} />
      </div>
