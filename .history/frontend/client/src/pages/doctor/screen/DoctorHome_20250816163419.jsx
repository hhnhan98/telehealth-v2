...
const DoctorHome = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        setLoading(true);
        const res = await axios.get('/doctors/dashboard'); // tương thích với BE
        const {
          todayAppointments = [],
          recentRecords = [],
          totalPatients = 0,
        ...
        setDashboard({ todayAppointments, recentRecords, totalPatients, weekSchedule });
        setError(null);
      } catch (err) {
          ...
          <table>
            ...
            <tbody>
              {dashboard.todayAppointments.map(appt => {
                const time = new Date(appt.date || appt.time).toLocaleTimeString([], {
                  hour: '2-digit',
                  minute: '2-digit'
                });
                let statusLabel = 'Mới';
                if (appt.status === 'confirmed') statusLabel = 'Đã xác nhận';
                else if (appt.status === 'cancelled' || appt.status === 'canceled') statusLabel = 'Đã hủy';
