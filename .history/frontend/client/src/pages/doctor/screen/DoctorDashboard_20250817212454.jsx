...
const DoctorDashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        setLoading(true);
        const res = await axios.get('/doctor/dashboard'); // chỉ cần /doctor/dashboard
        const { todayAppointments, recentRecords, totalPatients } = res.data;
  ...
  return (
    <div className="doctor-dashboard">
      ...
      {/* --- Bảng hồ sơ y tế gần đây --- */}
      <div className="recent-records">
        <h3>Hồ sơ y tế 7 ngày gần đây</h3>
        {recentRecords.length === 0 ? (
          ...
          <ul>
            {recentRecords.map(record => (
              <li key={record._id}>
                {record.patient?.fullName || 'Không có thông tin'} - {new Date(record.createdAt).toLocaleDateString()}
              </li>
            ))}
          </ul>
        )}
