...
export default function DoctorMedicalForm() {
  ...
  // ===== Load appointment từ route doctor =====
  useEffect(() => {
    const fetchAppointment = async () => {
      try {
        ...
        console.error(err);
        setError(err.response?.data?.message || err.message || 'Lỗi tải appointment');
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(`/api/appointments/${id}`, { status: 'completed' }, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  if (loading) return <p className="text-center mt-10 text-gray-600">Đang tải thông tin cuộc hẹn...</p>;
  if (error) return (
    <div className="text-center mt-10 text-red-500">
      <p>{error}</p>
      <Button onClick={() => navigate(-1)} className="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded">
        Quay lại
      </Button>
    </div>
  );
    ...
    <div className="max-w-3xl mx-auto p-4">
      ...
      <Card className="mb-6">
        ...
        <div className="space-y-2">
          <p><strong>Bệnh nhân:</strong> {appointment.patient?.fullName || 'Không rõ'}</p>
          <p><strong>Bác sĩ:</strong> {appointment.doctor?.fullName || 'Không rõ'}</p>
