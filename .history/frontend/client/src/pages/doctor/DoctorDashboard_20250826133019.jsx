...
const DoctorDashboard = () => {
  ...
  // Giả lập fetch API
  useEffect(() => {
    // TODO: thay bằng gọi API thật: /api/doctors/work-schedule?date=...
    setAppointments([
      {
        _id: "1",
        time: "09:00",
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {appointments.map((appt) => (
          <div
            key={appt._id}
            className={`p-4 rounded shadow cursor-pointer text-center transition hover:scale-105 ${
              appt.status === "booked"
                ? "bg-red-100 border border-red-300"
                : "bg-green-100 border border-green-300"
            }`}
            onClick={() => (appt.status === "booked" ? openModal(appt) : null)}
          >
