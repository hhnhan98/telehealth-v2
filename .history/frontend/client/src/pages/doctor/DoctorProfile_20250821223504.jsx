...
const DoctorProfile = () => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Chuyển birthYear thành number hoặc null nếu rỗng
      const birthYearToSend =
        profile.birthYear === '' ? null : profile.birthYear ?? profile.user.birthYear ?? null;
      ...
      // --- Merge user subdocument để FE hiển thị đúng birthYear ---
      setProfile(prev => ({
        ...prev,
        ...data.data,
        user: {
          ...prev.user,
          ...data.data.user,
          birthYear: data.data.user.birthYear ?? null, // đảm bảo luôn có field
        },
      }));
