...
const UserProfile = ({ currentUser, onProfileUpdate }) => {
  ...
  // Cập nhật form khi currentUser thay đổi - QUAN TRỌNG: Đảm bảo đồng bộ dữ liệu
  useEffect(() => {
    if (currentUser) {
      setUser(currentUser);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // QUAN TRỌNG: Gọi API để cập nhật thông tin
      ...
      // Lưu thông tin user mới vào localStorage để giữ trạng thái
  ...
  }, [formData.fullName, formData.phone, formData.gender, formData.birthYear, formData.address, isEditing, handleChange]);
  ...
  const DoctorFields = useMemo(() => {
    ...
    formData.workingHours, formData.consultationFee, formData.bio, specialties, isEditing, user.role, handleChange]);
  ...
  const PatientFields = useMemo(() => {
    ...
    formData.allergies, formData.medicalHistory, formData.insurance, isEditing, user.role, handleChange]);
  ...
  }, [formData.password, formData.confirmPassword, isEditing, handleChange]);
