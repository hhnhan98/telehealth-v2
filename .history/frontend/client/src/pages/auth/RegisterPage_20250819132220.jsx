...
const RegisterPage = () => {
  ...
  // Fetch specialties & locations khi mount
  useEffect(() => {
    let mounted = true;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.password || formData.password.length < 6) return alert('Mật khẩu phải có ít nhất 6 ký tự.');
    ...
    try {
      ...
      const msg = err?.message || err?.response?.data?.message || err?.response?.data?.error || 'Đã có lỗi xảy ra khi đăng ký.';
