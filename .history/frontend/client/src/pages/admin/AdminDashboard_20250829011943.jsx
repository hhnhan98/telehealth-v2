    ...
    if (type === 'doctor') {
      ...
      const attached = specialties
        .filter(s => {
          ...
          if (typeof s.location === 'string') return s.location === item._id;
          return s.location._id === item._id;
        })
        .map(s => s._id);
              ...
              return (
                <div key={f.key}>
                  ...
                  <select multiple name={f.key} value={form[f.key] || []} onChange={e => {
                    ...
                    {specialties.filter(s => !s.location || s.location === editId).map(opt => <option key={opt._id} value={opt._id}>{opt.name}</option>)}
                  </select>
