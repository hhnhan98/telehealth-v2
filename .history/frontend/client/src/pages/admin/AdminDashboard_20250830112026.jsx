...
const AdminDashboard = () => {
  ...
  const fetchAll = async () => {
    try {
      const [uRes, dRes, lRes, sRes, statRes] = await Promise.all([
  ...
  const handleChange = async e => {
    ...
    if (name === 'location' && value) {
      ...
      } catch (err) { console.error(err); setSpecialties([]); }
    }
  };
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === 'location') setForm({ name: item.name || '', address: item.address || '' });
  ...
  const handleResetPassword = async userId => {
    ...
    try { await axiosInstance.post(`/admin/users/${userId}/reset-password`); alert('Reset mật khẩu thành công'); }
    catch (err) { console.error(err); }
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setEditType(null); setEditId(null); setForm({}); setNewSpecialtyName('');
    } catch (err) { console.error(err); }
  };
  ...
  const handleAddSpecialty = async () => {
    ...
    try {
      const res = await axiosInstance.post('/admin/specialties', { name: newSpecialtyName, location: editId });
      setSpecialties(prev => [...prev, res.data.data]);
      setNewSpecialtyName('');
    } catch (err) { alert(err.response?.data?.message || 'Lỗi thêm Specialty'); console.error(err); }
  };

  const handleRemoveSpecialty = async (specialtyId) => {
    if (!window.confirm('Xác nhận xóa specialty khỏi location?')) return;
    try { await axiosInstance.delete(`/admin/specialties/${specialtyId}`);
      setSpecialties(prev => prev.filter(s => s && s._id !== specialtyId)); 
    } catch (err) { console.error(err); alert('Xóa specialty thất bại'); }
  };
  ...
  const formFields = {
    ...
    doctor: [
      { key: 'user', type: 'select', options: users.filter(u => u && u.role==='doctor').map(u => ({label:u.fullName||'-', value:u._id})) },
  ...
  const renderTable = (data, type, columns, page, setPage) => {
              ...
              return (
                <tr key={item._id}>
                  {columns.map(col => {
                    ...
                    else if(col==='specialties') value = specialties.filter(s => s.location?._id===item._id).map(s=>s.name).join(', ') || '-';
                    else value = item[col] ?? '-';
  ...
  // ===== Render Statistics (giữ nguyên logic trước) =====
  const renderStatistics = () => {
    if (!statistics) return <p>Đang tải thống kê...</p>;
    const statItems = [
      { label:'Locations', value:statistics.locations, color:'#3b82f6' },
      { label:'Specialties', value:statistics.specialties, color:'#10b981' },
      { label:'Doctors', value:statistics.doctors, color:'#f59e0b' },
      { label:'Patients', value:statistics.patients, color:'#8b5cf6' },
      { label:'Schedules', value:statistics.schedules, color:'#f43f5e' },
      { label:'Appointments', value:statistics.appointments, color:'#06b6d4' },
      { label:'Medical Records', value:statistics.medicalRecords, color:'#6366f1' },
    ];
    return (
      <div className="admin-section">
        <h3>Statistics</h3>
        <div className="statistics-grid">
          ...
          <div className="stat-card" style={{borderTop:`4px solid #f97316`}}>
            <div className="stat-label">Appointments Status</div>
            <ul>{Object.entries(statistics.appointmentStatusCounts).map(([status,count]) => <li key={status}>{status}: {count}</li>)}</ul>
          </div>
          <div className="stat-card" style={{borderTop:`4px solid #22c55e`}}>
            <div className="stat-label">Doctor Booking Counts</div>
            <ul>{Object.entries(statistics.doctorBookingCounts).map(([docId,count]) => {
              const doc = doctors.find(d => d._id === docId);
              return <li key={docId}>{doc?.user?.fullName || docId}: {count}</li>;
            })}</ul>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="admin-dashboard">
      ...
      {renderStatistics()}
      {renderTable(users,'user',['fullName','email','role','location','specialty'], pageUsers, setPageUsers)}
      {renderTable(doctors,'doctor',['user','specialty','location'], pageDoctors, setPageDoctors)}
      {renderTable(locations,'location',['name','address','specialties'], pageLocations, setPageLocations)}
              ...
              return (
                <div key={f.key}>
                  ...
                  <select name={f.key} value={form[f.key]||''} onChange={handleChange} required>
                    ...
                    {options.map((opt, idx) => !opt ? null : (typeof opt==='string' ? <option key={`${opt}-${idx}`} value={opt}>{opt}</option> : <option key={opt.key||`${opt.value}-${idx}`} value={opt.value}>{opt.label}</option>))}
                  </select>
                </div>
              );
            }
