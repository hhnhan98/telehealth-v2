...
const AdminDashboard = () => {
  ...
  const handleEdit = async (type, item) => {
    ...
    if (type === 'location') {
      ...
      // Fetch specialties theo location hiện tại cho doctor
      if (type === 'doctor' && copy.location) {
        try {
          const res = await axiosInstance.get(`/admin/specialties?location=${copy.location}`);
          setSpecialties(res.data.data || []);
        } catch (err) {
          console.error('Lỗi fetch specialties khi edit doctor:', err);
          setSpecialties([]);
        }
      }
      setForm(copy);
