...
const AdminDashboard = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      if (editId) await axiosInstance.put(`/admin/${editType}s/${editId}`, form);
      else await axiosInstance.post(`/admin/${editType}s`, form);
            ...
            {filtered.map(item => (
              <tr key={item._id}>
                {columns.map(col => {
                  ...
                  if (col === 'locations') value = (Array.isArray(item[col]) ? item[col] : [])
                    .map(id => locations.find(l => l._id === id)?.name).join(', ') || '-';
      ...
      <div className="admin-form card">
        ...
        <form onSubmit={handleSubmit}>
          {formFields[editType].map(f => {
            ...
            // --- Specialty filter by Location (for doctor) ---
