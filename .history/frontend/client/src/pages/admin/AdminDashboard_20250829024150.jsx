            ...
            <div style={{marginTop:15}}>
              ...
              <ul>
                {specialties.filter(s => s.location?._id === editId).map(s => (
                  <li key={s._id}>
                    {s.name} <button type="button" onClick={()=>handleRemoveSpecialty(s._id)}>Xóa</button>
                  </li>
                ))}
              </ul>
            </div>
            <div style={{marginTop:15}}>
              <label>Thêm Specialty</label>
              <input value={newSpecialtyName} onChange={e=>setNewSpecialtyName(e.target.value)} placeholder="Tên specialty mới"/>
              <button type="button" onClick={handleAddSpecialty}>Thêm</button>
            </div>
          </>}

          <div style={{marginTop:15,display:'flex',gap:10}}>
            <button style={{marginTop:15,display:'flex',gap:10}} type="submit">{editId?'Cập nhật':'Thêm'}</button>
