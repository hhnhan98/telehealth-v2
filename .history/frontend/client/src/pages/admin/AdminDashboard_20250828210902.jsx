...
const AdminDashboard = () => {
  ...
  const handleEdit = (type, item) => {
    ...
    setEditId(item._id);
    if (type === 'location') {
      const attached = specialties.filter(s => s.location && s.location._id === item._id).map(s => s._id);
  ...
  const formFields = {
    ...
    doctor: [
      ...
      { key: 'location', type: 'select', options: locations.map(l => ({ label: l.name, value: l._id })), required: true },
      { key: 'specialty', type: 'select', options: specialties.map(s => ({ label: s.name, value: s._id })), required: true },
    ],
    location: [
      { key: 'name', type: 'text' },
              ...
              <tr key={item._id}>
                {columns.map(col => {
                  ...
                  if (col === 'specialties' && Array.isArray(value))
                    value = specialties.filter(s => s.location === item._id).map(s => s.name).join(', ') || '-';
                  if (col === 'locations' && typeof value === 'object') value = value.name || '-';
