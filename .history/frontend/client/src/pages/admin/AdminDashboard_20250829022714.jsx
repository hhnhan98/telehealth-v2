...
const AdminDashboard = () => {
    ...
    if (type === 'doctor') {
      ...
      const attached = specialties
        .filter(s => {
          ...
          return s.location._id 
          ? s.location._id === item._id 
          : s.location === item._id;
        })
        .map(s => s._id);
  ...
  return (
    <div className="admin-dashboard">
      ...
      >
        <form onSubmit={handleSubmit}>
          {editType && formFields[editType].map(f => {
            ...
            // Multi-select (Specialty cho Location)
              ...
              return (
                <div key={f.key}>
                  ...
                  >
                    {specialties
                      .filter(s => {
                        // Nếu specialty chưa gắn Location thì giữ lại
                        if (!s.location) return true;
                        // Nếu s.location là object (populate) thì lấy _id so sánh
                        return s.location._id ? s.location._id === editId : s.location === editId;
                      })
                      .map(opt => (
                  ...
                  <button type="button" onClick={() => setIsSpecialtyModalOpen(true)}>Thêm Specialty mới</button>
                </div>
              );
            }
      ...
      {/* ===== Modal Thêm Specialty ===== */}
      <Modal
        isOpen={isSpecialtyModalOpen}
        title="Thêm Specialty"
        onClose={() => setIsSpecialtyModalOpen(false)}
        size="small"
      >
        <div>
          <label>Tên Specialty</label>
          <input value={newSpecialtyName} onChange={e => setNewSpecialtyName(e.target.value)} />
          <div style={{ marginTop: 10, display: 'flex', gap: 10 }}>
            <button onClick={handleAddSpecialty}>Thêm</button>
            <button onClick={() => setIsSpecialtyModalOpen(false)}>Hủy</button>
          </div>
        </div>
      </Modal>
    </div>
