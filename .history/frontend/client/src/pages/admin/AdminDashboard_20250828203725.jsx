...
const AdminDashboard = () => {
  ...
  const handleSubmit = async e => {
    e.preventDefault();
    try {
      const endpoint = editId ? `/admin/${pluralize(editType)}/${editId}` : `/admin/${pluralize(editType)}`;
      const res = editId
        ? await axiosInstance.put(endpoint, form)
        : await axiosInstance.post(endpoint, form);
      const data = res.data.data;

      if (editType === 'user') setUsers(prev => editId ? prev.map(u => u._id === editId ? data : u) : [...prev, data]);
  ...
  const renderTable = (data, type, columns) => {
    const filtered = data.filter(item =>
      !search || columns.some(c => {
        const value = item[c];
        if (typeof value === 'object' && value !== null) return JSON.stringify(value).toLowerCase().includes(search.toLowerCase());
              ...
              <tr key={item._id}>
                {columns.map(col => {
                  ...
                  if (col === 'locations' || col === 'specialties') {
                    value = (Array.isArray(item[col]) ? item[col] : [])
                      .map(id => specialties.find(s => s._id === id)?.name || locations.find(l => l._id === id)?.name || '')
                      .join(', ') || '-';
                  }
