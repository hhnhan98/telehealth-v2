...
const AdminDashboard = () => {
  ...
  const [editType, setEditType] = useState(null); // 'user' | 'doctor' | 'location' | 'specialty'
  ...
  const renderTable = (data,type,columns) => {
    const filtered = data.filter(item=>!search||columns.some(c=>item[c]?.toString().toLowerCase().includes(search.toLowerCase())));
    return (
            ...
            {filtered.map(item=>(
              <tr key={item._id}>
                {columns.map(col=>{
                  ...
                  if(col==='user') value = users.find(u=>u._id===item.user)?users.find(u=>u._id===item.user).fullName:'-';
      ...
      <div className="admin-form card">
        ...
        <form onSubmit={handleSubmit}>
          {formFields[editType].map(f=>{
            ...
            if(f.key==='specialty' && editType==='doctor' && form.location){
            ...
            if(f.type==='multiselect'){
              return (
                <div key={f.key}>
                  <label>{f.key}</label>
                  <select multiple name={f.key} value={form[f.key]||[]} onChange={handleChange}>
                    {options.map(opt=><option key={opt.value} value={opt.value}>{opt.label}</option>)}
                  </select>
                </div>
              )
  ...
  return (
    <div className="admin-dashboard">
      ...
      {renderTable(doctors,'doctor',['user','location','specialty','phone','bio'])}
