...
const AdminDashboard = () => {
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === 'location') {
      ...
      const copy = { ...item };
      if (copy.role === 'doctor' && copy.doctorInfo) {
        copy.location = copy.doctorInfo.location?._id || '';
        copy.specialty = copy.doctorInfo.specialty?._id || '';
      } else {
            ...
            {filtered.map(item => {
              ...
              return (
                <tr key={item._id}>
                  {columns.map(col => {
                    let value = '-';
                    if(col==='location') value = item.role==='doctor' ? item.doctorInfo?.location?.name || '-' : item.location?.name || '-';
                    else if(col==='specialty') value = item.role==='doctor' ? item.doctorInfo?.specialty?.name || '-' : item.specialty?.name || '-';
      ...
      >
        <form onSubmit={handleSubmit}>
          {editType && formFields[editType].map(f=>{
            ...
            if(f.type==='select') {
              const options = (editType==='user' && f.key==='specialty' && form.location)
