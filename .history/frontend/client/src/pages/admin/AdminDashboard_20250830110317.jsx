...
const AdminDashboard = () => {
  ...
  const [statistics, setStatistics] = useState(null); // <-- thêm state statistics
  ...
  const fetchAll = async () => {
    try {
      ...
      setStatistics(statRes.data.data || null); // <-- set statistics
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const formFields = {
    ...
    doctor: [
      { key: 'user', type: 'select', 
        options: users.filter(u => u && u.role==='doctor').map(u => ({label:u.fullName || '-', value:u._id}))
      },
  ...
  const renderStatistics = () => {
    ...
    return (
      <div className="admin-section">
        <h3>Statistics</h3>
        <div className="statistics-grid" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:15}}>
          <div className="stat-card">Locations: <strong>{statistics.locations}</strong></div>
          <div className="stat-card">Specialties: <strong>{statistics.specialties}</strong></div>
          <div className="stat-card">Doctors: <strong>{statistics.doctors}</strong></div>
          <div className="stat-card">Patients: <strong>{statistics.patients}</strong></div>
          <div className="stat-card">Schedules: <strong>{statistics.schedules}</strong></div>
          <div className="stat-card">Appointments: <strong>{statistics.appointments}</strong></div>
          <div className="stat-card">Medical Records: <strong>{statistics.medicalRecords}</strong></div>
          <div className="stat-card">
            Appointments Status:
            <ul style={{margin:0,paddingLeft:15}}>
              {Object.entries(statistics.appointmentStatusCounts).map(([status,count]) => (
          ...
          <div className="stat-card">
            ...
            <ul style={{margin:0,paddingLeft:15}}>
              {Object.entries(statistics.doctorBookingCounts).map(([docId,count]) => {
