...
const AdminDashboard = () => {
  ...
  const handleAdd = type => {
    setEditType(type); setEditId(null);
  ...
  const handleEdit = (type, item) => {
    setEditType(type); setEditId(item._id);
    if (type === 'location') {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setEditType(null); setEditId(null); setForm({});
    } catch (err) { console.error(err); }
  };
              ...
              <tr key={item._id}>
                {columns.map(col => {
                  ...
                  if (col === 'locations' || col === 'specialties') value = (Array.isArray(item[col]) ? item[col] : [])
                    .map(id => specialties.find(s => s._id === id)?.name || locations.find(l => l._id === id)?.name || '').join(', ') || '-';
