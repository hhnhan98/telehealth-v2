...
const AdminDashboard = () => {
  ...
  const handleEdit = (type, item) => {
    ...
    setEditId(item._id);
    if (type === 'location') {
      // thêm trường specialties để multi-select
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (editType === 'location' && Array.isArray(form.specialties)) {
        // Cập nhật mảng locations trong Specialty
        await Promise.all(form.specialties.map(async sId => {
          await axiosInstance.put(`/admin/specialties/${sId}`, { ...specialties.find(s => s._id === sId), locations: [form._id || form.specialties.includes(sId) ? sId : ''] });
        }));
      }
    ...
    return (
      <div className="admin-section">
        ...
        <table>
          ...
          <tbody>
            {filtered.map(item => {
              const attachedSpecialties = specialties.filter(s => Array.isArray(s.locations) && s.locations.includes(item._id));
              return (
                <tr key={item._id}>
                  {columns.map(col => {
                    let value = item[col] ?? '-';
                    ...
                    if (col === 'locations' || col === 'specialties') value = (Array.isArray(item[col]) ? item[col] : attachedSpecialties.map(s => s._id))
                      .map(id => {
      ...
      <div className="admin-form card">
        ...
        <form onSubmit={handleSubmit}>
          {formFields[editType].map(f => {
            ...
            // Doctor specialty filter by Location
            if (f.key === 'specialty' && form.role === 'doctor') {
              const availableSpecialties = specialties.filter(s =>
                Array.isArray(s.locations) && form.location && s.locations.includes(form.location)
              );
              return (
                <div key={f.key}>
