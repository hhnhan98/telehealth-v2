...
const AdminDashboard = () => {
  ...
  const handleChange = async e => {
    ...
    // Nếu chọn specialty, validate specialty-location
  ...
  const formFields = {
    ...
    doctor: [
      { key: 'user', type: 'select', options: users.filter(u => u.role && u.role==='doctor').map((u, idx) => ({label:u.fullName,value:u._id, key: `${u._id}-${idx}`})) },
      { key: 'location', type: 'select', options: locations.map((l, idx) => ({label:l.name,value:l._id, key:`${l._id}-${idx}`})) },
      { key: 'specialty', type: 'select', options: specialties.map((s, idx) => ({label:s.name,value:s._id, key:`${s._id}-${idx}`})) },
    ],
    location: [
      { key: 'name', type: 'text' },
