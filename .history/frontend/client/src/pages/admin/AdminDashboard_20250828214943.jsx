...
const AdminDashboard = () => {
  ...
  const fetchAll = async () => {
    try {
      const [uRes, dRes, lRes, sRes] = await Promise.all([
        ...
        axiosInstance.get('/api/admin/doctors'),
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === 'location') {
      const attached = specialties
        .filter(s => Array.isArray(s.locations) ? s.locations.includes(item._id) : s.location?._id === item._id)
        .map(s => s._id);
      setForm({ ...item, specialties: attached });
    } else if (type === 'specialty') {
      const locationId = Array.isArray(item.locations) ? item.locations[0] : item.location?._id || '';
      setForm({ ...item, location: locationId });
    } else {
      const copy = { ...item };
  ...
  const handleDelete = async (type, id) => {
    ...
    try {
      await axiosInstance.delete(`/api/admin/${pluralize(type)}/${id}`);
  ...
  const handleResetPassword = async userId => {
    ...
    try {
      await axiosInstance.post(`/api/admin/users/${userId}/reset-password`);
  ...
  const handleSubmit = async e => {
    ...
    try {
      const endpoint = editId
        ? `/api/admin/${pluralize(editType)}/${editId}`
        : `/api/admin/${pluralize(editType)}`;
      const res = editId
            ...
            {filtered.map(item => (
              <tr key={item._id}>
                {columns.map(col => {
                  ...
                  if (col === 'specialties') value = specialties
                    .filter(s => Array.isArray(s.locations) ? s.locations.includes(item._id) : s.location?._id === item._id)
                    .map(s => s.name)
                    .join(', ') || '-';
  ...
  return (
    <div className="admin-dashboard">
      ...
      <Modal
        ...
        size="medium"
      >// Trong phần form modal, thay đổi phần render select specialties cho Location
{editType && formFields[editType].map(f => {
  ...
  // Select input
  if (f.type === 'select') {
    // Multi-select cho Location -> specialties
    if (editType === 'location' && f.key === 'specialties') {
      return (
        <div key={f.key}>
          <label>{f.key}</label>
          <select
