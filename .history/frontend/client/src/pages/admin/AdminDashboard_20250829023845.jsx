...
const AdminDashboard = () => {
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === 'location') {
      const attached = specialties
        .filter(s => s.location?._id === item._id)
        .map(s => s._id);
  ...
  // ===== Specialty within Location =====
  const handleAddSpecialtyToLocation = async () => {
    if (!newSpecialtyName.trim() || !editId) return;
    ...
    try {
      ...
      setForm(prev => ({ ...prev, specialties: [...(prev.specialties || []), added._id] }));
      setNewSpecialtyName('');
    } catch (err) {
      console.error(err);
  ...
  const handleRemoveSpecialtyFromLocation = async (specialtyId) => {
    if (!window.confirm('Xác nhận xóa specialty này khỏi location?')) return;
    try {
      await axiosInstance.delete(`/admin/specialties/${specialtyId}`);
      setSpecialties(prev => prev.filter(s => s._id !== specialtyId));
      setForm(prev => ({ ...prev, specialties: prev.specialties.filter(id => id !== specialtyId) }));
    } catch (err) {
      console.error(err);
      alert('Lỗi xóa specialty');
    }
  };
    ...
    <div className="admin-dashboard">
      ...
      >
        <form onSubmit={handleSubmit}>
          ...
          {/* ===== Location Specialty Management ===== */}
          {editType === 'location' && (
            <div className="form-group">
              <label>Specialties hiện có</label>
              <div className="specialty-chips">
                {(form.specialties || []).map(id => {
                  const s = specialties.find(sp => sp._id === id);
                  if (!s) return null;
                  return (
                    <span key={id} className="chip">
                      {s.name} <button type="button" onClick={() => handleRemoveSpecialtyFromLocation(id)}>×</button>
                    </span>
                  );
                })}
              </div>
              <div className="add-specialty">
                <input
                  placeholder="Nhập tên Specialty mới"
                  value={newSpecialtyName}
                  onChange={e => setNewSpecialtyName(e.target.value)}
                  onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); handleAddSpecialtyToLocation(); } }}
                />
                <button type="button" onClick={handleAddSpecialtyToLocation} disabled={loadingSpecialty}>Thêm</button>
              </div>
            </div>
          )}
