...
const AdminDashboard = () => {
  ...
  // ===== Add / Remove Specialty from Location =====
  const handleAddSpecialty = async (locationId) => {
            ...
            {filtered.map(item=>(
              <tr key={item._id}>
                ...
                <td>
                  ...
                  {type==='location' && <>
                    <button onClick={()=>setEditId(item._id) || setIsSpecialtyModalOpen(true)}>Thêm Specialty</button>
                    {specialties.filter(s=>s.location?._id===item._id).map(s => (
                      <button key={s._id} onClick={()=>handleRemoveSpecialty(s._id)}>Xóa {s.name}</button>
                    ))}
                  </>}
                </td>
              </tr>
  ...
  return (
    <div className="admin-dashboard">
      ...
      {/* ===== Modal Thêm Specialty cho Location ===== */}
      <Modal
        isOpen={isSpecialtyModalOpen}
        title="Thêm Specialty cho Location"
      ...
      >
        <div>
          ...
          <div style={{marginTop:10,display:'flex',gap:10}}>
            <button onClick={()=>handleAddSpecialty(editId)}>Thêm</button>
