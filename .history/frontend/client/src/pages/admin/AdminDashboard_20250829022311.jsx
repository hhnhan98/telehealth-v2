...
const AdminDashboard = () => {
  ...
  const handleEdit = (type, item) => {
    ...
    if (type === 'location') {
      ...
      setForm({ name: item.name, description: item.description, location: item.location?._id || '' });
    }
  };
  ...
  const handleAddSpecialty = async () => {
    if (!newSpecialtyName.trim() || !editId) return alert('Nhập tên Specialty và chọn Location!');
    try {
            ...
            {filtered.map(item=>(
              <tr key={item._id}>
                ...
                <td>
                  ...
                  {type==='location' && <button onClick={()=>setEditType('specialty') || setEditId(item._id) || setIsSpecialtyModalOpen(true)}>Thêm Specialty</button>}
                </td>
              </tr>
            ))}
          </tbody>
