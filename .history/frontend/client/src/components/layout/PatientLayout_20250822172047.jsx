// import { useNavigate } from 'react-router-dom';
// import { Outlet, useLocation } from 'react-router-dom';
// import './SidebarLayout.css';

// function PatientLayout() {
//   const navigate = useNavigate();
//   const location = useLocation();

//   const getActive = (path) => location.pathname.includes(path);

//   const handleLogout = () => {
//     localStorage.removeItem('token');
//     navigate('/login');
//   };

//   return (
//     <div style={styles.wrapper}>
//       {/* Sidebar */}
//       <aside style={styles.sidebar}>
//         <h3 style={styles.title}>Telehealth System</h3>
//         <button
//           style={getActive('/dashboard') ? styles.activeBtn : styles.btn}
//           onClick={() => navigate('/patient/dashboard')}
//         >
//           Trang chủ
//         </button>
//         <button
//           style={getActive('/patient/doctors') ? styles.activeBtn : styles.btn}
//           onClick={() => navigate('/patient/doctors')}
//         >
//           Danh sách bác sĩ
//         </button>        
//         <button
//           style={getActive('/book-appointment') ? styles.activeBtn : styles.btn}
//           onClick={() => navigate('/patient/book-appointment')}
//         >
//           Đặt lịch hẹn
//         </button>
//         <button
//           style={getActive('/appointments') ? styles.activeBtn : styles.btn}
//           onClick={() => navigate('/patient/appointments')}
//         >
//           Lịch sử đặt lịch
//         </button>
//         <button
//           style={getActive('patient/profile') ? styles.activeBtn : styles.btn}
//           onClick={() => navigate('/patient/profile')}
//         >
//           Hồ sơ cá nhân
//         </button>
//         <button style={{ ...styles.btn }} onClick={handleLogout}>
//           Đăng xuất
//         </button>
//       </aside>

//       {/* Main content */}
//       <main style={styles.main}>
//         <Outlet />
//       </main>
//     </div>
//   );
// }

// export default PatientLayout;

// const styles = {
//   wrapper: {
//     display: 'flex',
//     minHeight: '100vh',
//     backgroundColor: '#F9FAFB'
//   },
//   sidebar: {
//     width: '220px',
//     padding: '15px',
//     backgroundColor: '#fff',
//     borderRight: '1px solid #E5E7EB',
//     display: 'flex',
//     flexDirection: 'column',
//     gap: '10px'
//   },
//   title: {
//     fontSize: '20px',
//     fontWeight: '600',
//     marginBottom: '20px',
//     color: '#111827'
//   },
//   btn: {
//     padding: '12px 16px',
//     backgroundColor: '#59c2ff6b',
//     color: '#111827',
//     border: 'none',
//     textAlign: 'left',
//     fontSize: '16px',
//     borderRadius: '6px',
//     cursor: 'pointer'
//   },
//   activeBtn: {
//     padding: '12px 16px',
//     backgroundColor: '#59c2ffff',
//     color: '#fff',
//     border: 'none',
//     textAlign: 'left',
//     fontSize: '16px',
//     borderRadius: '6px',
//     cursor: 'pointer'
//   },
//   main: {
//     flex: 1,
//     padding: '30px'
//   }
// };
import { useNavigate, Outlet, useLocation } from 'react-router-dom';
import './SidebarLayout.css';

function DoctorLayout() {
  const navigate = useNavigate();
  const location = useLocation();

  const getActive = (path) => location.pathname.includes(path);

  const handleLogout = () => {
    localStorage.removeItem('token');
    navigate('/login');
  };

  return (
    <div className="layout-wrapper">
      {/* Sidebar */}
      <aside className="layout-sidebar">
        <h3>Telehealth System</h3>

        <button
          className={`layout-btn ${getActive('/doctor/dashboard') ? 'active' : ''}`}
          onClick={() => navigate('/doctor/dashboard')}
        >
          Trang chủ
        </button>

        <button
          className={`layout-btn ${getActive('/doctor/patients') ? 'active' : ''}`}
          onClick={() => navigate('/doctor/patients')}
        >
          Danh sách bệnh nhân
        </button>

        <button
          className={`layout-btn ${getActive('/doctor/schedule') ? 'active' : ''}`}
          onClick={() => navigate('/doctor/schedule')}
        >
          Quản lý lịch
        </button>

        <button
          className={`layout-btn ${getActive('/doctor/appointments') ? 'active' : ''}`}
          onClick={() => navigate('/doctor/appointments')}
        >
          Lịch hẹn
        </button>

        <button
          className={`layout-btn ${getActive('/doctor/profile') ? 'active' : ''}`}
          onClick={() => navigate('/doctor/profile')}
        >
          Hồ sơ cá nhân
        </button>

        <button className="layout-btn" onClick={handleLogout}>
          Đăng xuất
        </button>
      </aside>

      {/* Main content */}
      <main className="layout-main">
        <Outlet />
      </main>
    </div>
  );
}

export default PatientLayout;
